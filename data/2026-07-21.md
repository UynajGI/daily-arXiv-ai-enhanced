...
- [physics.hist-ph](#physics.hist-ph) [Total: 5]
...
TL;DR: 提出了一种基于概率质量函数（PMF）的精确方法，用于模拟具有指定均值和成对相关性的依赖伯努利变量，解决了高斯阈值法的局限性，并提供了可行性判断与稀疏结构优化。


<details>
  <summary>Details</summary>
Motivation: 传统的高斯阈值方法在生成依赖伯努利变量时存在误差，尤其在相关性校准和正定性方面不准确，需要一种能精确匹配目标一阶和二阶矩的方法。

Method: 将问题直接建模为联合伯努利分布的概率质量函数上的线性规划，施加归一化、非负性、均值和成对交叉矩约束；同时提出凸包刻画、截断矩补全和稀疏支持集优化策略以提升可扩展性。

Result: 该方法能精确实现目标矩或认证不可行性，支持至多 $1+N+\binom{N}{2}$ 个状态的分布构造，不可行情况可通过二次分离证书验证；并提出了适用于中等维度和结构化实例的高效近似方案。

Conclusion: 基于PMF的框架优于传统高斯阈值方法，提供了关于可行性、精确模拟和结构限制的完整理论与实用工具，明确了现有方法的局限并给出精确替代方案。
...
TL;DR: 提出一种针对有限实验设计集的自适应贝叶斯最优实验设计算法，适用于昂贵计算模型，通过减少模型评估和方差提升效率。


<details>
  <summary>Details</summary>
Motivation: 在数据不足或不适当时，传统贝叶斯校准难以有效识别参数，且实际中实验设计变量常受限于有限选项，需更高效的实验设计选择方法。

Method: 结合加速嵌套蒙特卡洛估计器、重用参数样本、共用随机数和Rao-Blackwell化以降低方差，并利用bootstrap估计设计间优劣概率，迭代淘汰劣质设计。

Result: 算法能在显著降低计算成本的同时保持高可靠性，有效筛选出最优实验设计。

Conclusion: 该方法适合大规模工程应用中的贝叶斯最优实验设计，尤其适用于计算代价高的场景。
...
TL;DR: 本文提出了一种用于日本金融替代性纠纷解决（ADR）案件的统一数据集和建模框架，通过引入功能标记方案表示纠纷结构，并采用多任务模型联合进行纠纷分类与和解预测。实验表明，结合纠纷结构可提升预测性能，且大语言模型在多个领域表现优异，说明不同ADR领域间存在部分共享的纠纷结构。


<details>
  <summary>Details</summary>
Motivation: 旨在构建一个统一的数据集和建模框架，以捕捉和分析来自多个日本金融ADR机构的纠纷案例中的共性结构，从而提升对纠纷结果的预测能力。

Method: 提出一种功能标记方案来表示纠纷结构，并设计一个多任务模型，联合执行纠纷分类和和解结果预测；在多个真实ADR数据集上进行实验，评估模型性能。

Result: 实验结果显示，引入纠纷结构信息能够显著提升和解结果的预测性能；同时，大型语言模型在多个任务中达到或超过了传统模型的表现。

Conclusion: 纠纷结构在不同金融ADR领域中具有一定的通用性，利用结构化信息有助于提升纠纷结果预测效果，为跨领域纠纷分析提供了可行路径。
...
### [8] [Gaussian Reformulation of the Feynman Path Integral for Quantum Statistical Mechanics with Results for the Second Virial Coefficient of $^4$He](https://arxiv.org/abs/2607.16301)
*Phil Attard*

Main category: cond-mat.stat-mech

TL;DR: 将费曼路径积分重新表述为对每个位置构型邻域的高斯采样，利用环状聚合物统计和高温展开方法确定均值与方差，避免了传统量子蒙特卡洛方法中的多温节点和数值相消问题，并通过氦的第二维里系数验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 克服传统路径积分量子蒙特卡洛方法中因多温节点和统计平均中数值相消带来的计算难题，提高量子统计力学模拟的效率与精度。

Method: 将路径积分表示为基于高斯采样的邻域采样方法，结合环状聚合物在晶格上的统计特性以及Wigner-Kirkwood交换函数的高温展开来确定采样的均值和方差。

Result: 成功计算了氦的第二维里系数，模拟与解析结果均与实验测量值吻合良好。

Conclusion: 该方法提供了一种更稳定、高效的量子统计路径积分计算框架，适用于无需多温节点和避免数值不稳定的场景。
...
### [9] [Representation-Dependent Machine Learning of the Isotropic-Nematic Transition in the Lebwohl-Lasher Model](https://arxiv.org/abs/2607.16481)
*Maninder Kaur,Aojie Xue,David P Landau*

Main category: cond-mat.stat-mech

TL;DR: 该论文研究了在三维Lebwohl-Lasher模型中，机器学习检测相变时输入表示对称性保持的重要性。结果表明，无监督方法（如PCA和CAE）在使用原始分子构型时无法识别弱一阶各向同性-向列相变，但当输入转换为旋转不变的局域关联表示时，能成功检测到相变特征；而有监督的3D卷积神经网络（CNN）则可直接从原始构型准确预测序参量。


<details>
  <summary>Details</summary>
Motivation: 探讨输入表示是否保持系统对称性对机器学习检测相变的影响，特别是在具有取向有序且存在连续简并相的系统中。

Method: 采用主成分分析（PCA）、卷积自编码器（CAE）和有监督的三维卷积神经网络（CNN），分别处理原始分子构型和旋转不变的局域关联表示，比较其在检测Lebwohl-Lasher模型相变中的表现。

Result: PCA和CAE在原始构型下无法识别相变，但在旋转不变表示下成功检测到双峰共存分布和相变信号；有监督CNN能从原始构型准确回归序参量。

Conclusion: 对称性保持的输入表示对无监督机器学习在取向有序系统中的相变发现至关重要，而有监督学习可通过标签辅助克服表示缺陷。
...
### [10] [Glauber dynamics phase transitions in athermal random field Blume-Capel and Blume-Emery-Grifitths models](https://arxiv.org/abs/2607.16561)
*Sumedha,Aldrin B E*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了完全图上Glauber动力学与平衡态的两个模型，发现随机磁场方差R类似温度作用，存在临界值Rc使非平衡稳态与平衡态一致，并精确给出了相变位置。引入反铁双线性相互作用导致系统可能出现Rc=0，并在准静态演化中表现出向随机场Ising模型普适类的 crossover。外加磁场下，系统呈现多种磁滞回线形状，其面积依赖R，而形状由R=0时的行为决定。


<details>
  <summary>Details</summary>
Motivation: 探讨Glauber动力学在完全图上的非平衡稳态行为，比较其与热力学平衡态的差异，理解随机磁场方差R的作用及其对相变和磁滞现象的影响，特别是在引入 frustrations 和不同相互作用下的复杂相行为。

Method: 解析求解完全图上的Glauber动力学模型，包括Blume-Emery-Griffiths模型，在有无外加磁场和随机磁场的情况下分析其稳态性质，比较动力学稳态与平衡态，并推导磁滞回线的解析表达式。

Result: 发现随机场方差R起类似温度的作用，存在临界值Rc，当R>Rc时动力学稳态与平衡态一致；Rc可因晶体场调节变为零，并在演化中出现从Rc=0到√(2/π)的普适类 crossover；解析得出多种磁滞回线形状（矩形、六边形、蜂腰形等），其面积依赖R，形状由R=0时的模型行为决定；一阶相变区出现滞后现象。

Conclusion: Glauber动力学的稳态强烈依赖初始条件和R值，R在系统中扮演类似温度的角色；通过调控参数可实现不同普适类间的转变；磁滞行为丰富，其几何特征由系统在R=0时的拓扑决定，揭示了非平衡动力学中初始态和参数依赖的复杂相结构。
...
### [11] [On the Information Required for Feedback Control](https://arxiv.org/abs/2607.16639)
...
### [12] [Occupation-condensation transition of a sublinearly vertex-reinforced random walk on regular tree](https://arxiv.org/abs/2607.16971)
*Bon A Koo,Edward Ju*
...
### [13] [Floquet Driving of Enzymatic Reactions: Counting Statistics and Long-Time Currents](https://arxiv.org/abs/2607.17072)
...
### [14] [Richards' equation as a hydrodynamic limit: Chapman--Enskog reduction of the continuum kinetic equation for unsaturated soil water](https://arxiv.org/abs/2607.17358)
*Riccardo Rigon*
...
### [15] [Adverse Selection with Quality Variance: A Maximum-Entropy Approach](https://arxiv.org/abs/2607.17239)
...
### [16] [Large scale behavior in the Kuramoto-Sivashinsky equation: The Schwinger-Dyson route](https://arxiv.org/abs/2607.17915)
*Olivier Coquand*
...
### [17] [On the use of the Belopol'skaya-Daletskii representation of a diffusion on a Riemann manifold to construct path integrals](https://arxiv.org/abs/2607.17871)
...
### [18] [A Unified Discrete and Continuous Theory of Core-Halo Complexity Maximizers](https://arxiv.org/abs/2607.17907)
*Akshat Sharma*
...
### [19] [Expansion of a free Fermi gas released from an isotropic trapping potential](https://arxiv.org/abs/2607.18007)
...
### [20] [C3DIR: A Deep Learning 3-Dimensional Cloud Property Retrieval Scheme for Passive Satellite Imagers](https://arxiv.org/abs/2607.16929)
*Charles H. White,Yoo-Jeong Noh,John M. Haynes,Imme Ebert-Uphoff*

Main category: physics.ao-ph

TL;DR: C3DIR是一种基于深度学习的三维云特性反演模型，可匹配EarthCARE卫星数据，适用于多种被动成像仪，具备在航空、天气预报和气候研究中应用的潜力。


<details>
  <summary>Details</summary>
Motivation: 推动AI/ML驱动的三维云反演算法向业务化应用迈进，解决被动成像仪与主动探测仪器之间观测几何不匹配的问题。

Method: 开发了名为C3DIR的深度学习模型，采用体素级共定位方法，融合多源被动卫星成像仪数据，预测沿视线方向的冰、液态云和降雨的水含量分布，并构建三维云结构。

Result: 模型能准确识别多层重叠云系（略有平滑），在冰相水凝物检测和柱状积分水路径方面表现良好；但对薄层液态云的检测仍具挑战，液态水和雨水含量估算存在不确定性。

Conclusion: C3DIR能够提供灵活、垂直分辨的三维云结构输出，在云检测和水路径估算方面优于现有NOAA业务产品，具有广泛的应用前景。
...
### [21] [Satellite-based emissions estimate indicates progress toward China's methane mitigation goals](https://arxiv.org/abs/2607.16931)
...
TL;DR: 本研究利用TROPOMI卫星数据和区域大气反演模型，分析2019–2024年中国甲烷排放趋势，发现东部和中部地区年均增加0.3 Tg，增速放缓；煤炭行业排放强度首次下降，可能得益于政策干预；同时指出城市燃气扩张和农业排放上升为未来减排新挑战。


<details>
  <summary>Details</summary>
Motivation: 中国是全球最大的甲烷排放国，但近年来的排放趋势、来源及政策影响存在较大不确定性，亟需结合大气观测与政策分析以支持科学决策。

Method: 采用基于TROPOMI卫星观测的区域大气反演模型，量化2019–2024年中国每日甲烷排放，并结合行业政策进行分部门、次国家尺度和季节性分析。

Result: 2019–2024年，中国东部和中部地区甲烷排放年均增长0.3 Tg，增速较2010年代有所放缓；煤炭行业在产量上升背景下排放强度首次下降（-3.2% yr⁻¹），可能与多项政策工具有关；城市燃气使用扩展导致泄漏问题凸显，农业排放呈上升趋势但估算不确定性大。

Conclusion: 针对性的政策指令和激励措施可能有效推动甲烷减排，煤炭行业排放强度下降为其他 sector 提供借鉴，未来需重点关注能源转型中的城市燃气泄漏和农业排放控制。
...
### [22] [Smoothed Two-Stage Decomposition Algorithm for Solving Large-Scale Transmission and Distribution AC-OPF Problems](https://arxiv.org/abs/2607.16430)
*Juan Ospina,Manuel Garcia,Xinyi Luo,Andreas Wachter,David M. Fobes,Russell Bent*

Main category: math.OC

TL;DR: 本文提出了一种平滑两阶段分解优化器（StsDOpt），用于求解大规模、非凸的输配电网交流最优潮流（T&D AC-OPF）问题，通过主-子问题结构实现并行计算，无需近似或松弛，显著提升了计算效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 随着分布式能源资源（DERs）在配电网中的大量接入，传统将配电网视为静态负荷的最优潮流方法已不再适用，亟需精确建模配电网的非线性、多相、不平衡特性，以应对T&D联合系统带来的复杂性。

Method: 提出StsDOpt方法，采用两阶段分解框架，利用原对偶内点法的障碍函数特性，通过平滑技术使子问题响应对主问题变量可微，从而实现高效协同求解，并支持并行计算。

Result: 在PowerModelsITD框架中验证了StsDOpt的有效性，实验结果表明该方法显著减少了求解时间，提高了可扩展性，能够有效处理大规模T&D AC-OPF问题。

Conclusion: StsDOpt是一种鲁棒且可扩展的解决方案，能够在不依赖近似或松弛的情况下精确求解复杂的T&D AC-OPF问题，有助于DERs在输配电网中的可靠集成。
...
### [23] [Batteriesandthe BritishEnergy System](https://arxiv.org/abs/2607.16526)
*Waqquas Bukhsh*

Main category: math.OC

TL;DR: 本文探讨了电池在英国能源系统中的演变角色，强调其在实现净零目标中的重要性，并指出通过协调调度和更好整合电动汽车电池可释放更大价值。


<details>
  <summary>Details</summary>
Motivation: 随着能源系统的脱碳，电池在电力、交通和供热领域的作用日益重要，需要全面评估其当前价值及未来潜力。

Method: 通过分析国家能源系统运营商提供的未来情景数据，评估电池在电力系统和交通领域的贡献与挑战。

Result: 电动汽车可提供大部分储能容量，但无管理的充电行为可能给系统带来挑战；有效的市场设计和控制策略对释放电池价值至关重要。

Conclusion: 充分发挥电动汽车电池的价值需要协调调度、优化控制策略以及改进市场机制，以支持英国向2050年净零目标迈进。
...
### [24] [History-Dependent Recursive Preferences in Markov Decision Processes](https://arxiv.org/abs/2607.16538)
*William B. Haskell*
...
### [25] [Near-Optimal Lower Bounds for Exact Zeroth-Order Convex Optimization](https://arxiv.org/abs/2607.16558)
...
### [26] [End-to-End Supply Chain Planning in the Paper Industry Via Column Generation and Benders Decomposition](https://arxiv.org/abs/2607.16618)
...
### [27] [Dynamic mean-variance portfolio selection with no-shorting constraints and unknown investment opportunity sets](https://arxiv.org/abs/2607.16625)
...
### [28] [Dynamical Optimal Transport with $\mathfrak{so}(d)$-Invariance: From Theory to Computation](https://arxiv.org/abs/2607.16782)
...
### [29] [Tight Conic Relaxations for Rank-one Doubly Nonnegative Matrix Completion](https://arxiv.org/abs/2607.16796)
...
### [30] [Benchmarking Optimization Algorithms with Quality Profiles and Test Set Profiles](https://arxiv.org/abs/2607.16798)
...
### [31] [Supply Chain Networks](https://arxiv.org/abs/2607.17491)
*Elioth Sanabria*
...
### [32] [Treasure Search Optimization](https://arxiv.org/abs/2607.16863)
*A. Sharma*
...
### [33] [A Deep Reinforcement Learning Algorithm for the Vehicle Routing Problem with Stochastic Demands and Outsourcing](https://arxiv.org/abs/2607.16875)
...
### [34] [Entropic optimal transport need not select a zero-temperature limit](https://arxiv.org/abs/2607.16881)
*Maja Gwozdz*
...
### [35] [Robust Control for Marked Point Processes under Transition-Rate Uncertainty](https://arxiv.org/abs/2607.16935)
...
### [36] [Optimal Scheduling for Remote State Estimation over Hybrid Channels](https://arxiv.org/abs/2607.16950)
...
### [37] [Structural Vibration Control of Offshore Wind Turbines Using Tuned Mass Damper Inerter in OpenFAST: Implementation, Validation, and Illustration](https://arxiv.org/abs/2607.16962)
...
### [38] [Relative Entropy-Bounded Ambiguous Chance Constraints for Robust Planning in Nonlinear Systems](https://arxiv.org/abs/2607.16977)
...
### [39] [Real-World, Large Scale, Multi-Period Log Truck Routing and Scheduling : Application to Canadian Forestry](https://arxiv.org/abs/2607.16978)
...
### [40] [Robust Chance-Constrained Optimization using a Continuous Parameter Space Wasserstein-2 Ambiguity Set of Gaussian Mixtures](https://arxiv.org/abs/2607.17018)
...
### [41] [Optimal Combinatorial Testing with Constraints: The Balancing Act](https://arxiv.org/abs/2607.17083)
...
### [42] [Grid-Interactive Operation of Solar-Integrated Data Centers for Coordinated Local and System-Level Decarbonization](https://arxiv.org/abs/2607.17089)
...
### [43] [Dynamic Pricing for a Two-Sided Data Market Platform](https://arxiv.org/abs/2607.17119)
...
### [44] [A Parallel Evolutionary Algorithm Framework for Graph $k$-CUT Problems](https://arxiv.org/abs/2607.17158)
...
### [45] [A Principal-Agent Mean-Field Game Model of Insurance with Risk Interdependence](https://arxiv.org/abs/2607.17214)
...
### [46] [Uniform Exponential Stability Analysis of Impulsive Linear Time-Invariant Systems on Banach and Hilbert Spaces: Non-Coercive and Coercive Stability Conditions](https://arxiv.org/abs/2607.17260)
...
### [47] [Norm Minimisation Problems Involving Distances to Convex Sets](https://arxiv.org/abs/2607.17276)
...
### [48] [Solving Highly Constrained Multi-Objective Decision Problems with the IMAP-IGS Preference-Guided Metaheuristic Framework](https://arxiv.org/abs/2607.17304)
...
### [49] [From a Scalar to a Matrix Setting for the Dai--Liao Parameter](https://arxiv.org/abs/2607.17337)
...
### [50] [CADMM-Prox: A Bi-level Consensus ADMM for Non-smooth Non-convex Distributed Consensus Optimization](https://arxiv.org/abs/2607.17495)
...
### [51] [Consumption and Investment in Incomplete Markets with Epstein-Zin Preferences in Infinite Horizon](https://arxiv.org/abs/2607.17557)
*Ho Man Tai*
...
### [52] [Feedback Cycles in Exploratory Equilibria](https://arxiv.org/abs/2607.18128)
*Chen-Hung Wu*
...
### [53] [Monotonicity and Frank-Wolfe Dynamics in Atomic Splittable Congestion Games](https://arxiv.org/abs/2607.17684)
*Tobias Harks*
...
### [54] [Limiting Stationarity of Regularized Gap-Function Reformulations for Bilevel Optimization with Unbounded Multipliers](https://arxiv.org/abs/2607.17772)
...
### [55] [Thermal management optimization of Battery Electric Vehicles via hierarchical NMPC with CMO-trained Neural Models](https://arxiv.org/abs/2607.17892)
...
### [56] [Finding Fair Draws for Incomplete Round Robin Tournaments](https://arxiv.org/abs/2607.17901)
...
### [57] [Reflected Schrodinger Bridge Problem over Sub-Riemannian Manifold](https://arxiv.org/abs/2607.17904)
*Daniel Owusu Adu*
...
### [58] [A Curvature-Aware Rank-Adaptive Distributed Augmented-Lagrangian Solver for Large-Scale SDPs](https://arxiv.org/abs/2607.17933)
...
### [59] [Long-time behavior and turnpike properties of linear-quadratic graphon mean field control problems](https://arxiv.org/abs/2607.18000)
...
### [60] [Improved Convergence Rate for Stochastic Multi-Gradient Descent: A Proof Discovered with AI](https://arxiv.org/abs/2607.18174)
*Lisha Chen*
...
### [61] [Risk-sensitive exit-time control for stochastic differential equations with path-dependent coefficients](https://arxiv.org/abs/2607.18192)
...
### [66] [Planning for Mission Efficiency, Robustness, and Resilience for Unmanned Autonomous Systems (UASs) in Contested Environments](https://arxiv.org/abs/2607.17085)
*Michael J. Gaiewski,Sergey N. Vecherin,Laurel P. Williams,Igor Linkov*
...
### [67] [The unintended consequences of large language models as a labor-augmenting technology in science](https://arxiv.org/abs/2607.17397)
...
### [69] [A principled closure framework for higher-order SIS epidemic models on networks](https://arxiv.org/abs/2607.18003)
...
### [70] [Mapping the DAG-ness Landscape: Structural Archetypes in Complex Networks](https://arxiv.org/abs/2607.16464)
*Erik Csikos*

Main category: cs.SI

TL;DR: 该论文提出并实证评估了“DAG-性”框架，通过四个维度量化有向网络的结构特性，在107个跨领域网络中发现系统普遍趋向四种结构性原型，揭示了复杂系统中超越学科界限的普适性结构规律。


<details>
  <summary>Details</summary>
Motivation: 传统对有向网络的二元分类（是有向无环图与否）掩盖了真实系统中丰富的层次、循环和模块化结构，缺乏统一的分析框架来揭示其深层组织原则。

Method: 提出一个包含无环性、流向一致性、循环局部性和路径复杂性四个组件的DAG-性度量框架，并在来自12个不同领域的107个真实网络数据集上进行实证分析。

Result: 发现不同领域网络不按学科聚类，而是汇聚为四种普适的结构原型；即使存在丰富反馈的系统也普遍存在宏观无环性；如神经连接组与信息网络等差异巨大的系统表现出相同的拓扑约束。

Conclusion: DAG-性提供了一个统一、可解释且与领域无关的视角，有助于揭示复杂系统中有向结构的隐藏规律。
...
### [71] [Auditing Differential Visibility of Political Content on TikTok](https://arxiv.org/abs/2607.17356)
*Hazem Ibrahim,Tewoflos Girmay*

Main category: cs.SI

TL;DR: 该研究通过分析55万多个视频数据，检验TikTok是否对特定政治内容进行“隐形降权”，发现看似存在的内容可见性差异实际上是由于伪重复和混杂因素导致，而非平台审查；在账户层面未发现中等到大幅的内容压制证据。


<details>
  <summary>Details</summary>
Motivation: 针对TikTok被指控隐藏或压制政治内容的广泛质疑，本研究旨在通过实证方法检验这些指控是否成立，并揭示表面现象背后的统计误判与结构性偏差。

Method: 研究构建了一个包含2,753个视频、67个账号的高密度小时级面板数据集，覆盖三个争议话题（美国移民执法、特朗普报道、以色列/巴勒斯坦），使用多步分类器识别主题相关视频，立场由账号所属阵营确定；比较传统聚合分析与账户层面独立单元分析的结果差异，并进行多重检验校正与功效分析，验证结果稳健性。

Result: 聚合分析显示话题条件下的传播差距极显著（p < 10^-140），但在账户层面分析中该差距消失，所有对比均不显著（BH-FDR q接近0.9）；未发现中等到大效应的传播压制；相反，在反特朗普和支持巴勒斯坦内容中发现更高的每观看互动率（Cliff's delta = -0.51 和 -0.64；q < 0.03），表明设计能检测到效应但实际无压制。

Conclusion: 所谓‘隐形降权’是伪重复（将自相关的视频小时视为独立观察）和混杂变量（如被指受压制一方规模更大、多用阿拉伯语发布）造成的假象；更合理的解释是某些立场的内容受众更具参与度；研究强调可信的可见性审计需避免统计陷阱并以正确单位进行分析。
...
### [72] [Dynamical system for PageRank with a time-dependent memory](https://arxiv.org/abs/2607.17953)
*Julio Flores,Eva Primo,Daniel Rodríguez,Miguel Romance*

Main category: cs.SI

TL;DR: 提出了一种连续时间PageRank模型，其中个性化向量根据记忆函数动态演化；证明了在强连通网络中多数记忆函数导致收敛到与初值无关的稳态，而指数振荡记忆函数则引发渐近周期行为。


<details>
  <summary>Details</summary>
Motivation: 受Gleich和Rossi动态PageRank框架启发，旨在研究记忆效应如何影响PageRank排名的长期行为，特别是当个性化向量依赖于历史信息时。

Method: 建立了一个基于积分微分方程的连续时间PageRank模型，个性化向量的记忆由加权平均过去值决定；利用积分微分方程理论分析解的存在唯一性，并研究不同记忆函数对长时间行为的影响。

Result: 对于强连通网络，大多数记忆函数使PageRank向量收敛至与初始条件无关的稳态；但当记忆函数为指数振荡型（如e^{at}cos(bt)）时，系统呈现渐近周期性行为。

Conclusion: 记忆函数的选择显著影响PageRank的动态演化：一般记忆导致稳定排序，而振荡记忆可引发持续波动，表明记忆机制能从根本上改变排名过程的定性行为。
...
### [73] [A family of mixed-mixed strongly conservative finite element methods for Biot's model of consolidation](https://arxiv.org/abs/2607.16418)
*Qingguo Hong,Johannes Kraus,Maria Lymbery*

Main category: math.NA

TL;DR: 本文研究了Biot固结模型的四场弱形式，提出了一类保持角动量和流体质量平衡的混合有限元方法，并证明了离散问题的适定性和最优先验误差估计。


<details>
  <summary>Details</summary>
Motivation: 为了解决Biot准静态固结模型中多物理场耦合问题，需要构造既能保持物理守恒性质又具有良好数学适定性的数值方法。

Method: 采用基于H(div div)-协调有限元的混合-混合有限元方法，保留角动量和流体质量平衡的强形式，并在适当的Hilbert空间框架下分析连续和离散系统的适定性。

Result: 证明了连续系统在所选Hilbert空间中的适定性，提出了保持物理守恒律的有限元格式，验证了离散问题的可解性并获得了最优的先验误差估计。

Conclusion: 该方法能有效求解四场Biot模型，同时保持关键物理量的守恒性，具有良好的理论基础和收敛性能。
...
### [74] [Convergence and stability of truncated Euler-Maruyama algorithm for stochastic proportional delay Mckean-Vlasov models with jump process](https://arxiv.org/abs/2607.16438)
*Amr Abosenna,Zhuoqi Liu*

Main category: math.NA

TL;DR: 本文研究了带有Lévy跳跃和比例延迟的随机McKean-Vlasov模型，允许非跳跃项超线性增长，并应用截断Euler-Maruyama算法分析其收敛性和几乎必然指数稳定性，通过数值例子验证理论结果。


<details>
  <summary>Details</summary>
Motivation: 由于McKean-Vlasov模型在金融、生物和控制等领域具有广泛应用，且实际中存在延迟和跳跃现象，因此研究带有比例延迟和Lévy跳跃的超线性增长模型具有重要意义。

Method: 采用截断Euler-Maruyama算法对模型进行数值逼近，并分析该算法的收敛速率和几乎必然指数稳定性。

Result: 证明了截断Euler-Maruyama算法在所考虑模型下的收敛性，并建立了几乎必然指数稳定性的条件，数值实验验证了理论分析的有效性。

Conclusion: 截断Euler-Maruyama方法适用于求解具有比例延迟和Lévy跳跃的超线性McKean-Vlasov随机微分方程，且能保持良好的稳定性和收敛性。
...
### [75] [Surrogate-to-code verification of a non-intrusive POD-GPR machine-learning emulator of peak thermomechanical fields, with application to a carbon-carbon aircraft brake disc](https://arxiv.org/abs/2607.16447)
*Franklin Kamche*

Main category: math.NA

TL;DR: 提出了一种验证框架，用于评估非侵入式降阶模型在有限元仿真中的预测精度，确保其误差低于数值离散化不确定性，实现了代理模型与高保真求解器之间的保真度一致性。


<details>
  <summary>Details</summary>
Motivation: 为了满足认证级应用对误差评估的严格要求，需要将降阶模型的预测误差与其所基于的有限元求解器的离散化不确定性进行比较，而非仅依赖物理观测。

Method: 结合本征正交分解（POD）和高斯过程回归（GPR），构建非侵入式降阶模型，并提出一种代数上精确的后验误差分析方法，分离截断误差和回归误差；通过网格收敛性研究确定离散化误差下限，并以此评估模型精度。

Result: 在独立测试集上，该模型对峰值温度和等效应力的预测均落在高保真求解器自身离散化不确定范围内，达到“代理-代码”保真度；残余误差主要来源于线性降维限制而非回归误差。

Conclusion: 所提框架可有效验证非侵入式降阶模型在工程认证场景下的可靠性，适用于任何能获得离散化误差估计的参数化有限元仿真问题。
...
### [76] [Finite element exponential integration for rough solutions of nonlinear wave equations. Part I: Dirichlet boundary conditions on polygonal and polyhedral domains](https://arxiv.org/abs/2607.16511)
...
### [77] [Augmented Lagrangian preconditioning for a simplified Ericksen--Leslie model of nematic liquid crystals](https://arxiv.org/abs/2607.16628)
...
### [78] [A Deep Second-Order Stochastic Residual Method for Fully Nonlinear Parabolic PDEs](https://arxiv.org/abs/2607.16730)
...
### [79] [Analytic regularity for a fourth-order singularly perturbed boundary balue problem with two small parameters](https://arxiv.org/abs/2607.16781)
...
### [80] [PolyChopper: a Polyhedron Splitting Scheme](https://arxiv.org/abs/2607.16887)
...
### [81] [Neural operator preconditioning from mixed dataset for the Helmholtz equations: Application to transcranial ultrasound](https://arxiv.org/abs/2607.16906)
*Yanfei Xiang*
...
### [82] [Novel Adaptive Methods for Hyperbolic Conservation Laws Based on New Quasi-Linear Seventh- and Ninth-Order Schemes](https://arxiv.org/abs/2607.16918)
...
### [83] [A Linear Variable-Step Embedded ETD Scheme with Uniform-in-Time Stability for the 2D Navier--Stokes Equations](https://arxiv.org/abs/2607.17036)
...
### [84] [Convergence of Finite Element Methods for Ricci Flow](https://arxiv.org/abs/2607.17051)
...
### [85] [Digital Nets on Cubature Nodes: Inheriting Cubature Accuracy on Low-Dimensional Projections](https://arxiv.org/abs/2607.17080)
*Takehito Yoshiki*
...
### [86] [A Lowest-Order Robust Mixed Finite Element Method with a Third-Order Tensor Variable for Strain Gradient Elasticity](https://arxiv.org/abs/2607.17134)
...
### [87] [A Rational Discrete Collocation Method for Second Kind Fredholm Equations](https://arxiv.org/abs/2607.17179)
...
### [88] [A Dynamical Approximation Scheme on the Stiefel manifold for Wasserstein Gradient Flows](https://arxiv.org/abs/2607.17235)
...
### [89] [Raviart--Thomas Elements with Geometric Correction for Distorted Quadrilateral Meshes](https://arxiv.org/abs/2607.17285)
*So-Hsiang Chou*
...
### [90] [Stability and Robustness Analysis of Regularized Reconstruction Methods for Low-Dose Computed Tomography in Parallel-Beam Geometry](https://arxiv.org/abs/2607.17298)
*Mohamed Berrada*
...
### [91] [Iterated graph Laplacian for image restoration problems](https://arxiv.org/abs/2607.17313)
...
### [92] [FEVessel: Mesh-Independent Analysis of 3D Pressure Vessels with the Label-Free Pretrained Finite Element Method](https://arxiv.org/abs/2607.17318)
...
### [93] [A posteriori error estimates for parabolic PDEs on evolving surfaces](https://arxiv.org/abs/2607.17416)
*Michael Lantelme*
...
### [94] [An operator-splitting algorithm for the hypergraph $p$-Laplacian with applications to missing data recovery](https://arxiv.org/abs/2607.17606)
...
### [95] [A domain decomposition online-learning-enhanced nonlinear elimination preconditioner](https://arxiv.org/abs/2607.17723)
...
### [96] [Pathwise skew-symmetric discretisation for SDEs with superlinear drift](https://arxiv.org/abs/2607.17735)
...
### [97] [Field-of-values analysis of augmented Krylov methods for matrix $\varphi$-function actions](https://arxiv.org/abs/2607.17819)
...
### [98] [Cubature from rational approximation](https://arxiv.org/abs/2607.17851)
*Gentian Zavalani*
...
### [99] [Hybrid-Dimensional Biot Problem with an Optimization Based Domain Decomposition Approach](https://arxiv.org/abs/2607.17885)
...
### [100] [A Direct Approach to Hermite Interpolation](https://arxiv.org/abs/2607.17998)
...
### [101] [Uniform-in-time rational approximation of the matrix exponential with real poles](https://arxiv.org/abs/2607.18018)
...
### [102] [Krasnosel'skii-Mann iterations beyond asymptotics: a combinatorial analysis](https://arxiv.org/abs/2607.18121)
...
### [103] [The conditional higher moment risk measure: second-order asymptotics with FGM contagion](https://arxiv.org/abs/2607.16601)
*Haifan Hu,Bingzhen Geng,Jiajun Liu,Shijie Wang*

Main category: q-fin.RM

TL;DR: 本文研究了在Farlie-Gumbel-Morgenstern（FGM）依赖结构下条件高阶矩（CoHM）一致风险度量的二阶渐近展开，适用于描述主损失风险与参考风险之间的弱传染效应。基于极值理论和二阶正则变化理论，针对属于Fréchet、Weibull或Gumbel极大值域吸引子的主风险，系统推导出二阶渐近展开式。与现有的一阶结果相比，所提出的二阶逼近能更准确地捕捉尾部行为和相依性影响。数值模拟表明，二阶渐近方法在极端置信水平下显著降低了近似误差，保险索赔数据的应用进一步验证了其实际优越性。


<details>
  <summary>Details</summary>
Motivation: 为了更精确地度量具有弱相依结构的金融或保险风险中的高阶风险特征，尤其是在极端事件下的风险估计精度不足的问题，本文旨在发展条件高阶矩风险度量的二阶渐近理论，以改进现有的一阶近似方法。

Method: 采用极值理论和二阶正则变化理论，在Farlie-Gumbel-Morgenstern（FGM）相依结构假设下，对属于Fréchet、Weibull和Gumbel极大值域吸引子的主风险构建条件高阶矩（CoHM）一致风险度量的二阶渐近展开。通过数学推导获得渐近表达式，并利用数值模拟和真实保险索赔数据进行验证。

Result: 成功推导出三种极大值域吸引子下的二阶渐近展开式，结果显示该方法比一阶近似更精确地反映尾部相依性和高阶风险特征；数值实验表明在极端置信水平下近似误差显著降低；实证分析证实该方法在真实保险数据中具有更好的适用性和预测性能。

Conclusion: 本文提出的二阶渐近方法在刻画弱相依风险结构下的极端风险时优于传统一阶方法，提升了条件高阶矩风险度量的精度，具有重要的理论意义和实际应用价值，尤其适用于保险和金融领域的极端风险管理。
...
### [104] [Determining Insolvency Regions in Banks: A Stochastic Dynamic Approach Integrating Liquidity and Credit Risk](https://arxiv.org/abs/2607.17381)
*Nader Karimi,Davood Ahmadian*

Main category: q-fin.RM

TL;DR: 本文构建了一个连续时间结构动态模型，用于确定银行因流动性与信用风险非线性交互而导致的精确资不抵债区域，并结合巴塞尔III监管要求，提出可用于实时监控的代理函数。


<details>
  <summary>Details</summary>
Motivation: 现有文献多将流动性风险与信用风险孤立处理或采用简化形式建模，缺乏对两者非线性反馈机制及其导致内生性破产的研究，尤其是在新兴市场背景下。

Method: 建立包含LCR和NSFR等巴塞尔III监管要求的连续时间结构模型，运用随机最优控制框架求解HJB方程以确定精确的资不抵债边界，并推导出可实时监测的代理解析近似函数。

Result: 模型揭示了流动性压力与信贷违约共同发生时存在显著的非线性阈值效应，会不成比例地加速银行走向资不抵债；所提出的代理函数在伊朗银行业数据校准下表现出良好的实用性。

Conclusion: 该研究为理解新兴市场的金融摩擦提供了新视角，提出了一个严格的综合风险管理框架，可为监管机构提供高效的实时压力测试与早期预警工具。
...
### [105] [Scalar glueball-$s\bar{s}$ mixing in one flavor lattice QCD](https://arxiv.org/abs/2607.17294)
*Long-Cheng Gui,Wei Sun,Ying Chen,Ming Gong,Geng Li,Zhaofeng Liu*

Main category: hep-lat

TL;DR: 研究了在N_f=1格点QCD中最低标量胶球与s\bar{s}介子的混合，利用广义本征值问题方法确定了两个最低本征态的质量，并发现存在显著的混合角和较大的混合能量，表明标量 sector 中胶球与s\bar{s}介子之间存在强混合。


<details>
  <summary>Details</summary>
Motivation: 理解实验上同位旋标量标量介子的本质需要非微扰输入，因此研究标量胶球与s\bar{s}介子之间的混合具有重要意义。

Method: 在各向异性的16^3×128格点系综上使用N_f=1格点量子色动力学，通过求解优化后的胶球和s\bar{s}标量算符在J^PC=0++通道中的广义本征值问题来提取质量本征态及其耦合强度。

Result: 测得两个最低本征态的质量分别为m1 = 1.290(20) GeV和m2 = 1.777(49) GeV，混合角|θ|≈40.7(2.7)°，混合能量xs = 239(24) MeV，显示出强烈的胶球-s\bar{s}混合。

Conclusion: 标量 sector 中胶球与s\bar{s}介子之间存在强混合，为理解实验上的同位旋标量介子提供了重要的非微扰信息，未来需进一步研究混合能量的连续极限和夸克质量依赖性。
...
### [106] [Renormalization of meson susceptibilities and RG-invariant symmetry ratios in QCD](https://arxiv.org/abs/2607.17816)
*Ting-Wai Chiu*

Main category: hep-lat

TL;DR: 本文分析了具有精确手征对称性的格点QCD中有限温度下介子关联函数的紫外发散结构，指出通过对称性比κ_{AB}可实现重整化群不变和方案无关的物理量构造，并讨论了与威尔逊费米子的不同发散行为。


<details>
  <summary>Details</summary>
Motivation: 研究在具有精确手征对称性的格点QCD中，有限温度下介子关联函数的紫外发散结构，特别是如何通过温度减除和对称性保护来消除发散并构建物理上有意义的可观测量。

Method: 采用具有精确手征对称性的格点公式，分析裸关联函数中的加性发散和乘性重整化因子Z_Γ²；利用温度减除法去除温度无关的发散项，并构造对称伙伴之间的比值κ_{AB}以实现重整化群不变性。

Result: 发现加性发放在温度减除后被消除，乘性因子Z_A = Z_B在质量无关方案中成立，使得κ_{AB}是重整化群不变且方案无关的；而威尔逊费米子由于显式破坏手征对称性，导致Z_A ≠ Z_B，破坏该构造。

Conclusion: 精确手征对称性可禁止质量相关的幂发散，保证Z_A = Z_B，从而使对称性比κ_{AB}成为可靠的、可用于研究恢复对称性和U(1)_A异常效应的物理量。
...
### [107] [A Novel Hybrid Quantum Reservoir Computing (nHQRC) for Phase Transition Detection in Non-Equilibrium Dynamical Systems](https://arxiv.org/abs/2607.16281)
*Manoj B. Bhatkar,Prashant M. Yawalkar*

Main category: quant-ph

TL;DR: 提出了一种新型混合量子储备计算框架（nHQRC），用于高效检测非平衡动力系统中的相变，避免传统变分量子算法的梯度消失和训练开销问题。


<details>
  <summary>Details</summary>
Motivation: 传统变分量子算法在处理高度非线性随机数据时受限于梯度消失、贫瘠高原问题和高昂训练成本，难以有效检测非平衡系统中的隐含相变。

Method: 采用冻结的无序横场伊辛模型（TFIM）将时变随机驱动力映射到指数级希尔伯特空间，并引入前瞻-free的预放大流形缩放技术以解决相位多缠绕问题；通过遗传算法优化多量子比特配置至‘混沌边缘’，利用冯·诺依曼熵和量子费舍尔信息作为纠缠见证进行量子态追踪，结合随机薛定谔桥（SSB）生成轨迹预测。

Result: 在8维非平稳随机驱动场下，该框架相比经典基准将系统漂移-扩散效率显著提升，并减少最大轨迹衰减超过13%。

Conclusion: nHQRC提供了一种具有O(1)时间开销的鲁棒方案，适用于近期量子设备中的宏观子系统稳定与相变早期预警。
...
### [108] [QR-SPPS: Quantum-Native Retail Supply Chain Risk Simulation via VQE, ADAPT-VQE Counterfactual Policy Ranking, and DOS-QPE Boltzmann Tail Risk Quantification](https://arxiv.org/abs/2607.16275)
*Sumit Tapas Chongder*

Main category: quant-ph

TL;DR: 本文提出了一种基于量子计算的零售供应链风险分析框架QR-SPPS，利用伊辛哈密顿量建模多层级供应商网络中的级联故障，并通过VQE、ADAPT-VQE和DOS-QPE等量子算法实现相关风险传播、政策评估与系统性尾部风险量化，展示了在工业规模问题上超越经典模拟的潜力。


<details>
  <summary>Details</summary>
Motivation: 传统供应链风险模型假设节点故障相互独立，难以捕捉多级供应商网络中由依赖关系导致的关联性级联失效，从而低估系统性风险。本文旨在利用量子计算天然处理复杂关联性的能力，构建更准确的风险分析框架。

Method: 将40节点四层供应链网络映射为40量子比特的伊辛哈密顿量，使用ZZ耦合项表示供应商间的相关依赖；采用硬件高效的变分量子本征求解器（VQE）计算压力基态；引入ADAPT-VQE梯度筛选进行反事实政策评估；并通过DOS-QPE重建能谱，结合市场波动温度估计玻尔兹曼加权灾难概率。

Result: QR-SPPS揭示了纠缠的级联故障模式，显著不同于经典蒙特卡洛方法的预测结果；实现了六种危机干预措施的实时排序而无需重复优化；提供了兼容VaR分析的量子原生尾部风险度量；验证了该框架在相关风险传播、政策优化和系统性风险量化方面的有效性。

Conclusion: 该研究展示了量子原生算法在供应链风险管理中的实际应用潜力，特别是在处理大规模关联故障时相较经典方法具有指数级优势，为未来在真实工业场景中部署量子风险模拟器奠定了基础。
...
### [109] [Relativistic bound-state solutions for a non-central Schi{ö}berg-type hyperbolic potential with double ring-shaped angular terms](https://arxiv.org/abs/2607.16265)
*Shakir. M. Nagiyev,Vatan H. Badalov*

Main category: quant-ph

TL;DR: 研究了非中心Schiöberg型双环形角势垒下Klein-Gordon方程的奇异相互作用对量子谱的影响，通过精确求解角向方程和近似处理径向方程，揭示了奇异性对波函数域和能级结构的调控机制。


<details>
  <summary>Details</summary>
Motivation: 探索奇异相互作用如何通过改变允许的波函数域来重塑相对论性量子系统中的能谱结构，特别是在具有复杂角势垒的情形下的物理机制。

Method: 在标量与矢量耦合相等的情况下分离径向与角向动力学，精确求解角向Klein-Gordon方程，并利用Greene-Aldrich近似处理径向方程，得到Jacobi多项式形式的波函数及隐式相对论量子化条件。

Result: 发现赤道面上的反平方奇异性将构型空间划分为具有相同角谱的反射对称区域；仅在一个区域内移除势垒时，只保留奇宇称态；零势垒极限下恢复非相对论Schrödinger谱；对NaH和Na₂分子的低振动能级计算结果与实验基本一致，但近解离区存在偏差。

Conclusion: 奇异相互作用可通过限制波函数定义域显著影响相对论量子系统的能谱和宇称选择规则，该模型在描述真实分子振动谱方面具有一定有效性，但在渐近行为上存在近似局限。
...
### [110] [Intelligence-Guided Adaptive Purification for DDoS-Resilient Quantum Networks: A CUDA-Q based Study](https://arxiv.org/abs/2607.16276)
*Santanu Ganguly*

Main category: quant-ph

TL;DR: 本文提出了一种CUDA-Q/SeQUeNCe协同仿真工作流，用于研究异构线性中继链中的自适应纠缠纯化策略，并表明网络异常感知的控制器可通过权衡吞吐量与保真度来提升量子网络的有效输出。


<details>
  <summary>Details</summary>
Motivation: 量子中继网络在经典控制平面受到网络异常或拒绝服务攻击影响时，面临纠缠生成率、端到端保真度、纯化开销和存储延迟之间的复杂权衡，需要具备适应性的控制策略。

Method: 开发了基于CUDA-Q和SeQUeNCe的协同仿真框架：CUDA-Q模拟噪声量子核以评估基本的纠缠纯化与交换行为，SeQUeNCe构建事件级模型模拟随机链路生成、存储退相干、纯化失败和端到端交换过程；对比多种纯化策略，并引入基于CSE-CIC-IDS2018入侵检测数据的异常评分机制，实现对攻击状态的感知与响应。

Result: 在8节点链中，资源惩罚型风险感知策略相比固定纯化策略提升了目标以上纠缠交付概率，同时降低延迟与纯化开销；在攻击期间，无感知控制器的目标以上交付概率下降至0.098±0.007，而IDS感知的自适应控制器通过增强纯化将该值提升至0.344±0.011，接近理想值0.335。

Conclusion: 具备网络安全状态感知能力的控制器能动态调整资源分配，在控制平面受损时仍可维持较高性能，证明了跨层安全信息融合对提升实用化量子网络鲁棒性的重要价值。
...
### [111] [EPIC-CIM: Training Convolutional Neural Networks on a Coherent Ising Machine via Equilibrium Propagation](https://arxiv.org/abs/2607.16271)
*Xingrui Yin,Shenwei Kang,Haoqi He,Yan Xiao,Hongdong Zhu,Hai Wei,Yin Ma,Qi Gao,Xiaochun Cao,Kai Wen*

Main category: quant-ph

TL;DR: 提出基于能量学习的量子卷积神经网络训练框架，通过能量最小化避免梯度回传，适用于非可微和离散量子系统。


<details>
  <summary>Details</summary>
Motivation: 解决量子卷积神经网络因非可微操作和离散动力学导致的传统梯度训练困难问题。

Method: 采用能量模型框架，将输入数据通过卷积和量子采样处理，利用自由相与弱钳制相之间的能量差驱动参数更新，无需显式梯度计算。

Result: 实现了在非可微和离散空间中的稳定一致学习，增强了模型的物理解释性和可观测性。

Conclusion: 该框架为量子卷积神经网络提供了统一的物理视角，促进了量子计算与人工智能的有效融合。
...
### [112] [Entanglement geometry separates circuit cutting, classical hardness, and trainability](https://arxiv.org/abs/2607.17872)
*Maria Gragera Garces,Sabina Drăgoi,Lirandë Pira*

Main category: quant-ph

TL;DR: 本文研究了量子电路切割中的纠缠几何对量子优势、经典模拟难度和可训练性的影响，指出矩阵乘积态和树状张量网络电路在低采样开销下仍可高效经典模拟，难以实现渐近量子优势；通过分离控制边界和块内纠缠，构造出可廉价切割但具有超多项式MPS键维的电路，但MPS难度与可训练性在深度上存在冲突；而以魔法（magic）作为硬度资源可避免此冲突，浅层Clifford+T电路在保持可切割和可训练的同时，其稳定子模拟成本随T门数量指数增长。


<details>
  <summary>Details</summary>
Motivation: 为了在当前硬件限制下实现可扩展的量子计算，需要理解电路切割的开销、经典模拟的难度以及变分量子算法的可训练性如何受到纠缠结构的影响。

Method: 通过分析矩阵乘积态（MPS）和树张量网络（TTN）电路的纠缠几何特性，构建具有可控边界和块内纠缠的双块电路模型，并数值验证至n=100；同时探讨以魔法（非稳定子资源）替代纠缠作为计算硬度来源的可能性。

Result: MPS和TTN类电路虽可低开销切割，但可高效经典模拟，无法实现渐近量子优势；新构造的双块电路可在保持低切割开销的同时具备超多项式全局MPS键维；然而MPS硬度要求深度d=ω(log n)，而可训练性要求d=O(log n)，二者冲突；使用Clifford+T电路中T门数量带来的魔法可避免该冲突，实现指数级增长的模拟难度同时保持可训练性。

Conclusion: 纠缠几何严重制约了基于纠缠的量子优势实现路径，而将魔法作为硬度资源为在低深度、可切割、可训练电路中实现经典难模拟提供了更有前景的方向。
...
### [113] [Variance-Reduced Trajectory Unravelings for GPU Noisy Quantum-Circuit Simulation: Characterization and a Qiskit-Aer Integration Gap](https://arxiv.org/abs/2607.17678)
*Chun-Yeol You*
...
### [114] [A Hybrid Classical-Quantum Approach for Multi-Constrained Location Optimization Problem](https://arxiv.org/abs/2607.16277)
...
### [115] [Memory effects in pulsed optomechanical systems](https://arxiv.org/abs/2506.03455)
...
### [116] [Number Fluctuations and Entanglement-Spectrum Participation in Monitored Free Fermions](https://arxiv.org/abs/2607.16278)
...
### [117] [The Quantum Correction to Gaussian Information Geometry is the Killing Form of the Symplectic Algebra](https://arxiv.org/abs/2607.16376)
*Christian Kerskens*
...
### [118] [Hamiltonian Lift of Bures--Wasserstein Covariance Dynamics with a Spectral Floor](https://arxiv.org/abs/2607.16380)
*Christian Kerskens*
...
### [119] [The Norton Theorem for Quantum Circuits and Systems](https://arxiv.org/abs/2607.16385)
...
### [120] [Sample-based quantum diagonalization approach for open-shell transition-metal complexes in gas and implicit-solvent](https://arxiv.org/abs/2607.16389)
...
### [121] [Dissipative Stabilization of Floquet-Engineered Many-Body Order](https://arxiv.org/abs/2607.16391)
...
### [122] [$p$-Body $\simeq$ Range $p-1$: Exact Order-Range Mapping and Dual-Unitarity](https://arxiv.org/abs/2607.16642)
*Tanay Pathak*
...
### [123] [Entanglement Entropy in Quantum Networks with Tunable Geometry](https://arxiv.org/abs/2607.16394)
...
### [124] [Strong Quantum Mpemba Effect from Exact Slow-Mode Selection in Constrained Rydberg Chains](https://arxiv.org/abs/2607.17975)
...
### [125] [Uncountably many inequivalent maximally entangled measurements for two qutrits](https://arxiv.org/abs/2607.16396)
...
### [126] [Quantum-Centric Geometry Optimization with Wave-Function-Based Embedding](https://arxiv.org/abs/2607.16410)
...
### [127] [Remote Infrared Absorption Spectroscopy with Undetected Photons](https://arxiv.org/abs/2607.16419)
...
### [128] [Feynman Meets Turing: The Curse of Quantum Universality](https://arxiv.org/abs/2607.16436)
...
### [129] [Host-guest Crystal Engineering Tailors the Room Temperature Spin Dynamics in Molecular Quantum Devices](https://arxiv.org/abs/2607.16466)
...
### [130] [Holographic quantum codes with trapped ions](https://arxiv.org/abs/2607.16503)
...
### [131] [Split-quaternionic structure and canonical quantization of the Bateman dual oscillator](https://arxiv.org/abs/2607.16516)
*Mathieu Beau*
...
### [132] [Efficient Exact Quantum Sampling from the Sun-Wootters Distribution for Optimal Polynomial Intersection](https://arxiv.org/abs/2607.16541)
*Sunghyeon Jo*
...
### [133] [Quantum dynamics of a levitated ferromagnetic gyroscope](https://arxiv.org/abs/2607.16592)
...
### [134] [Geometric quantum discord in the black hole quantum atmosphere](https://arxiv.org/abs/2607.16641)
...
### [135] [Geometric Power Capacity of Coherent Ergotropy in Quantum Batteries](https://arxiv.org/abs/2607.16645)
...
### [136] [Mean-State Entropy Hierarchies and Classical Communication through Quantum Convolutions](https://arxiv.org/abs/2607.16653)
...
### [137] [Graph State Generation Based on Quantum Photonic Devices, Enabling Measurement-Based Quantum Computing](https://arxiv.org/abs/2607.16667)
*Masoud Hakimi Heris*
...
### [138] [A projection-free approach toward mapping the structured polarization fields](https://arxiv.org/abs/2607.16672)
...
### [139] [Quantum Metrological Detection of the Unruh Effect with Optimized Accelerated Two-Level Atoms](https://arxiv.org/abs/2607.16701)
*Yao Jin*
...
### [140] [Reliability Is Not Free in Universal Quantum Work Extraction](https://arxiv.org/abs/2607.16702)
*Shuai Zeng*
...
### [141] [Extending the Frontiers of QNLP Beyond English: Grammar-Sensitive Pipeline for Hindi Sentiment Classification Using Compositional Quantum Models](https://arxiv.org/abs/2607.16765)
...
### [142] [Identity-Paired Progressive Depth Training: When Trainability Persists Beyond Expressibility](https://arxiv.org/abs/2607.16800)
...
### [143] [Shortcuts to adiabaticity in five-level systems using counter-diabatic driving and time-rescaling optimization](https://arxiv.org/abs/2607.16829)
...
### [144] [Removing inhomogeneous broadening in cross-relaxation spectra via hole burning](https://arxiv.org/abs/2607.16839)
...
### [145] [Geometric Qubits in Programmable Atomic Trimers](https://arxiv.org/abs/2607.16846)
...
### [146] [Attosecond delay metrology beyond the photon coherence time with spectrally resolved Hong-Ou-Mandel interferometry](https://arxiv.org/abs/2607.16849)
...
### [147] [Universal Parent Hamiltonians for Adiabatic Warm Starts](https://arxiv.org/abs/2607.16857)
...
### [148] [Experimental Nanoscale Thermodynamics](https://arxiv.org/abs/2607.16861)
...
### [149] [Certified Optimal Measurement Reduction over Quantum Context Landscapes](https://arxiv.org/abs/2607.16866)
...
### [150] [Beyond Orbital Rotations: Correlation-Rank Limits and Clifford-Accessible Measurement, from Algebra and Global Optimization](https://arxiv.org/abs/2607.16869)
*Federico Zahariev,Vanda Glezakou*
...
### [151] [Bargmann invariants and local unitary equivalence](https://arxiv.org/abs/2607.16878)
*Minghui Ma,Rui Shi*
...
### [152] [Precision dynamics of resonantly enhanced optical parametric amplifiers](https://arxiv.org/abs/2607.16885)
*Evan D. Hall*
...
### [153] [A Reponse to the Sheldon Goldstein Objection to a Series of Covariant Relativistic Bohmian Models](https://arxiv.org/abs/2607.16928)
...
### [154] [Bound, Scattering, and Resonance States of the Symmetric Woods--Saxon Potential in Dunkl Quantum Mechanic](https://arxiv.org/abs/2607.16944)
...
### [155] [Operational Relation Between One-Time and Two-Time Work Protocols and Measurement-Resolution-Induced Transitions in Quantum Work Statistics](https://arxiv.org/abs/2607.16951)
...
### [156] [Entanglement Quantification via Symmetric Extensions: A Resource Theory Hierarchy](https://arxiv.org/abs/2607.16960)
...
### [157] [How many degrees of freedom describe a quantum N-particle state?](https://arxiv.org/abs/2607.16988)
...
### [158] [Encoding Choices and Fault-Tolerant Resource Estimates for Digital Quantum Hamiltonian Descent](https://arxiv.org/abs/2607.16996)
...
### [159] [Rigorous characterization of continuous-variable quantum states via optical parametric amplifiers](https://arxiv.org/abs/2607.17023)
...
### [160] [quchip: A Differentiable Toolkit for Modeling Quantum Devices](https://arxiv.org/abs/2607.17081)
*Ibraheem AlYousef*
...
### [161] [Against Many Worlds](https://arxiv.org/abs/2607.17086)
...
### [162] [GHZ-Equivalent State Distribution in Quantum Networks: Reducing Decoherence and Quantum Resource Consumption](https://arxiv.org/abs/2607.17101)
...
### [163] [Blind Transpiler: An open-source library for universally blind and homomorphic quantum computations](https://arxiv.org/abs/2607.17131)
...
### [164] [QBism Logic](https://arxiv.org/abs/2607.17174)
*Kenji Tokuo*
...
### [165] [Multi-level Random-Telegraph Noise Mitigation using a Single Spectator Qubit](https://arxiv.org/abs/2607.17202)
...
### [166] [Heralded Leakage Detection with Preserved Computational-State Coherence in a Fixed-Frequency Transmon](https://arxiv.org/abs/2607.17204)
...
### [167] [Relativistic quantum teleportation protected by the anti-Unruh effect](https://arxiv.org/abs/2607.17216)
...
### [168] [Continuous Time Quantum Walk Propagation for Irregular Temporal Graph Forecasting](https://arxiv.org/abs/2607.17273)
...
### [169] [Truncated Wigner approximation for spins in continuous phase space](https://arxiv.org/abs/2607.17295)
...
### [170] [Chiral Entangled-State Generation through Dissipative Quantum Dynamics](https://arxiv.org/abs/2607.17302)
...
### [171] [Constraints on recovering quantum information after erasure](https://arxiv.org/abs/2607.17319)
...
### [172] [Scaling law for optimal excitation storage and superradiant release in waveguide QED systems](https://arxiv.org/abs/2607.17320)
...
### [173] [Locally Scrambled Quantum Memories for Loss-Tolerant Entanglement-Assisted Optical Interferometry](https://arxiv.org/abs/2607.17321)
*Jianqi Sheng*
...
### [174] [Interpreting Quantum Learning Models via Stochastic Processes](https://arxiv.org/abs/2607.17327)
...
### [175] [Broadband Polarization Compensation with Link Segment Reconstruction for Quantum Optical Links](https://arxiv.org/abs/2607.17400)
...
### [176] [Operator-centric Clifford algebra for variational eigensolvers and finite-shot adaptive selection](https://arxiv.org/abs/2607.17443)
...
### [177] [The finite key effect of side-channel-secure quantum key distribution beyond post-selection technique](https://arxiv.org/abs/2607.17465)
...
### [178] [LLM-Driven Cross-Paradigm Design for Quantum Optimal Control](https://arxiv.org/abs/2607.17498)
...
### [179] [Neural Gauge-P Representation for Open Quantum Dynamics of Interacting Bosons](https://arxiv.org/abs/2607.17534)
...
### [180] [Lie-Group Mode Connectivity in Quantum Machine Learning from a Dynamical Lie Algebra Perspective](https://arxiv.org/abs/2607.17554)
*Hiroshi Ohno*
...
### [181] [Temporal Diffraction Grating for Engineered Superconducting Qubit Dissipation](https://arxiv.org/abs/2607.17562)
...
### [182] [Single-atom sensor for low-frequency electric field](https://arxiv.org/abs/2607.17583)
...
### [183] [Spatial nonlocality imaging via metasurface](https://arxiv.org/abs/2607.17618)
...
### [184] [Fully-connected three-mode squeezed vacuum: Gaussian entanglement, steering, and collective photon subtraction](https://arxiv.org/abs/2607.17629)
...
### [185] [Active Optical Frequency Measurements with Superradiance Prolonged by a Modulated Magnetic Field](https://arxiv.org/abs/2607.17647)
...
### [186] [Quantum Key Distribution Beyond Stationary Channels](https://arxiv.org/abs/2607.17690)
...
### [187] [Image Classification on IBM Quantum Computers](https://arxiv.org/abs/2607.17705)
...
### [188] [Formal Verification of Continuous-Variable Quantum Programs](https://arxiv.org/abs/2607.17714)
...
### [189] [Light-Cone Scaling of In-Circuit Noise in Randomized Measurements](https://arxiv.org/abs/2607.17740)
...
### [190] [Geometry-Resolved Projection of RF Imbalance to Ion Micromotion in a Same-Phase Dual-RF Blade Trap](https://arxiv.org/abs/2607.17793)
...
### [191] [Stochastic Pauli-path simulator for large-scale quantum optimization](https://arxiv.org/abs/2607.17804)
...
### [192] [High-frequency dual-channel lock-in detection via rapidly oscillating driving](https://arxiv.org/abs/2607.17854)
...
### [193] [Unifying Charge-Learnability Transitions in U(1)-Symmetric Quantum Circuits through Informational Power of Local Measurement](https://arxiv.org/abs/2607.17886)
...
### [194] [How the Quantum Sorites Phenomenon Strengthens the Bell Argument and How a Random-Matrix Collapse Dynamics Answers It](https://arxiv.org/abs/2607.17894)
*Malcolm Forster*
...
### [195] [Sensing relativistic quantum fields with minimally perturbing local measurements](https://arxiv.org/abs/2607.17920)
...
### [196] [Variational non-gaussian approach to interacting spin-boson models](https://arxiv.org/abs/2607.17934)
...
### [197] [Noise structuring in fixed-depth Trotter simulation: stationary channels and observable-level depolarization](https://arxiv.org/abs/2607.17936)
...
### [198] [New bound on $S_{1}\times S_{2}$-setting Bell locality of a nonseparable Werner state](https://arxiv.org/abs/2607.18050)
*Elena R. Loubenets*
...
### [199] [Topology of the Set of Entangled State](https://arxiv.org/abs/2607.18105)
...
### [200] [Exponential Reduction of Mesh Dependence in Quantum Estimation of Parabolic PDE Observables](https://arxiv.org/abs/2607.18113)
*Xiantao Li*
...
### [201] [CutBackdoor: A Circuit Cut Triggered Backdoor Attack on Variational Quantum Algorithms](https://arxiv.org/abs/2607.18126)
...
### [202] [Phase-Sensitive Benchmarking of Composite Quantum Gates with Chiral-Interference Circuits on Quantum Hardware](https://arxiv.org/abs/2607.18137)
...
### [203] [Hardware Robustness of Sample-Based Quantum Diagonalization](https://arxiv.org/abs/2607.18196)
...
### [204] [QuantiSpect: A Structure-Aware Lightweight 3D CNN Pre-Decoder for Scalable Surface Code Quantum Error Correction](https://arxiv.org/abs/2607.18204)
...
### [205] [An Interactive Interface for Control Integration in Mid-Fidelity Wind Farm Simulation](https://arxiv.org/abs/2607.16386)
*Zekai Chen,Casey Heiskell,Ryozo Nagamune*

Main category: eess.SY

TL;DR: 本文介绍了一种名为ffconnect的开源Python接口，用于FAST.Farm风力发电场模拟器，支持更智能的风电场控制设计与验证。


<details>
  <summary>Details</summary>
Motivation: 为了支持数据驱动和人工智能技术在风电场控制中的应用，需要一个能够交互式设计和验证的仿真工具。

Method: 开发了一个基于Python的通用接口ffconnect，重构了API并增强了状态访问能力，完全集成于Python环境以支持现代科学计算和机器学习生态系统。

Result: 实验表明，ffconnect在不同模拟长度和风电场规模下相比原版FAST.Farm几乎没有运行时开销，并通过偏航跟踪案例研究证明了其有效性。

Conclusion: ffconnect是一个高效、开放且易于使用的工具，有助于推动智能风电场控制系统的发展与应用。
...
### [206] [A 0.74-mW K$_u$-band Cryogenic SiGe LNA in 130-nm BiCMOS with 14-K Noise Temperature for Scalable Quantum Readout](https://arxiv.org/abs/2607.16505)
*Xiuhan Chen,Haoling Li,Gun Suer,Leonardo Ranzani,Kin Chung Fong,Aravind Nagulu,Najme Ebrahimi*

Main category: eess.SY

TL;DR: 本文提出了一种工作在K$_u$波段的亚毫瓦低温低噪声放大器（LNA），采用GF 130CBIC SiGe BiCMOS工艺实现，适用于集成在Still级的亚开尔文量子比特读出链。该四级级联LNA采用巴伦耦合级间匹配、Q增强谐振槽和负$g_m$交叉耦合结构，在2.5 K下测得14 K的平均噪声温度和28 dB增益，功耗仅为0.74 mW，据作者所知是目前报道中K$_u$波段低温SiGe LNA最低功耗。


<details>
  <summary>Details</summary>
Motivation: 为了满足亚开尔文量子比特读出系统对极低功耗和高灵敏度的需求，尤其是在低温环境下集成约瑟夫森参量放大器（JPA）和SiGe LNA的挑战，需要开发一种在保持高性能的同时显著降低直流功耗的低温LNA。

Method: 设计了一款四阶段共源共栅结构的LNA，采用巴伦耦合实现级间匹配，引入Q值增强的谐振槽并结合2比特开关进行频率调谐，同时使用电压控制的交叉耦合负$g_m$单元以优化噪声与增益性能；并对Q单元在不同温区的工作点进行优化以确保带内性能最优且工作稳定。

Result: 在2.5 K条件下测试，LNA在12–18 GHz范围内实现约14 K的平均噪声温度，峰值增益达28 dB（中心频率约16.5 GHz），直流功耗仅0.74 mW。

Conclusion: 该LNA实现了K$_u$波段低温SiGe放大器中最低的直流功耗，同时保持优异的噪声性能和增益，适合用于高度集成的低温量子读出系统，为未来极低温电子学提供了高能效解决方案。
...
### [207] [Distributed Adaptive Estimation of Unknown Nonlinear Systems without Input Sharing](https://arxiv.org/abs/2607.16510)
*Moh Kamalul Wafi,Milad Siami*

Main category: eess.SY

TL;DR: 本文研究了在有向通信网络下离散时间非线性系统的分布式自适应状态估计问题，提出了一种归一化自适应估计方案，能够在无共享激励或控制输入的情况下实现鲁棒的状态和动态识别，并通过李雅普诺夫分析和LMI条件保证估计误差的稳定性与收敛性。


<details>
  <summary>Details</summary>
Motivation: 针对未知源动态的非线性系统，在分布式传感网络中缺乏全局信息和共享激励的情况下，实现准确且鲁棒的状态估计具有挑战性，本文旨在解决该问题。

Method: 提出一种归一化的自适应估计算法，各智能体仅利用本地测量和邻居信息进行状态估计；通过李雅普诺夫方法分析估计误差的输入到状态稳定性（ISS），并推导基于范数和LMI的Schur稳定条件以刻画网络耦合影响。

Result: 在无扰动情况下估计误差渐近收敛，在有界扰动下参数保持有界；仿真验证了在星型、环形和路径拓扑下的估计准确性，并证实所提方法具有良好可扩展性。

Conclusion: 所提分布式自适应方案无需共享控制或激励即可有效估计非线性系统状态，具备良好的鲁棒性与网络适应性，适用于大规模传感网络。
...
### [208] [A Unified Statistical Framework for Multicopter Propeller Damage Diagnosis Based on Functionally Pooled Models and Bayesian Quantification: Experimental Flight Test Assessment](https://arxiv.org/abs/2607.16512)
...
### [209] [Comparative Analysis of Linepack Impact in Hydrogen and Natural Gas Networks under Dynamic Operating Conditions](https://arxiv.org/abs/2607.16527)
...
### [210] [Dynamic Speed Limit Control of Connected Automated Vehicles in Freeway Networks Considering Traffic Composition Uncertainty](https://arxiv.org/abs/2607.16615)
...
### [211] [Cooperative MARL-Based Energy-Efficient Power Control for Two-Hop Relaying Networks](https://arxiv.org/abs/2607.16703)
...
### [212] [Driver Behavior Under Traffic Complexity: Variance Decomposition and Cross-Driver Generalization for Driver Monitoring](https://arxiv.org/abs/2607.16847)
...
### [213] [Learning to Stay Fresh: A Self-Learning Semantic Framework for Underwater Internet of Things](https://arxiv.org/abs/2607.16902)
...
### [214] [A conditional no-go for resource-free magic-axis measurement on a static surface code](https://arxiv.org/abs/2607.16968)
...
### [215] [Cluster-Based Distributed Small-Signal Stability Certificates for Grid-Forming Inverter Networks](https://arxiv.org/abs/2607.16985)
...
### [216] [Increasing Line Outage Localization Performance with Ensemble Classifiers](https://arxiv.org/abs/2607.17008)
...
### [217] [Feasibility-Aware Security-Constrained Unit Commitment via Hybrid Soft Actor-Critic with Quantum-Sampled Features](https://arxiv.org/abs/2607.17031)
...
### [218] [Optimal Safety Control using High-Order Control Barrier Functions](https://arxiv.org/abs/2607.17032)
...
### [219] [A recursive subspace based method for errors-in-variables model identification of time-varying systems](https://arxiv.org/abs/2607.17065)
...
### [220] [Deep Reinforcement Learning-Based Energy Management for Hydrogen-Enabled Community Microgrids Under Uncertainty](https://arxiv.org/abs/2607.17248)
...
### [221] [A Phased Development Framework Enabling Islanded Operation of Sustainable AI Data Centers With Onsite Grid-Following and Grid-Forming Energy Architectures](https://arxiv.org/abs/2607.17391)
...
### [222] [An Update to the Level Set Theorems in Hamilton-Jacobi Reachability Analysis](https://arxiv.org/abs/2607.17435)
...
### [223] [Multi-scale closed-loop melt pool control for LPBF via policy optimization](https://arxiv.org/abs/2607.17438)
...
### [224] [Topology-Aware Propagation-Based Assessment of Extreme-Weather Impacts on Distribution System Resilience](https://arxiv.org/abs/2607.17565)
...
### [225] [Online learning of neural state-space models](https://arxiv.org/abs/2607.17614)
...
### [226] [On Optimal Event-Triggered Distributed Control for Stochastic Multi-Agent Systems via Reinforcement Learning](https://arxiv.org/abs/2607.17635)
...
### [227] [Graph-Induced Tensor Liftings for Networked SEIR Models: Dimensional Reduction and Residual Analysis](https://arxiv.org/abs/2607.17664)
*Enrique Baeyens*
...
### [228] [Multisine electrochemical impedance spectroscopy for Li-ion battery characterisation](https://arxiv.org/abs/2607.17832)
...
### [229] [A Compact 3D-Printed Soft Finger with Cyclic Hydraulic Actuation](https://arxiv.org/abs/2607.17840)
...
### [230] [An Augmented Load-Frequency Control Block Diagram for Power Systems](https://arxiv.org/abs/2607.17873)
...
### [231] [Real-Time Flight Test Maneuver Selection with Monte Carlo Tree Search](https://arxiv.org/abs/2607.18089)
...
### [232] [LLMs and Agentic AI Systems for Smart Grids: A Tutorial on Architectures and Applications](https://arxiv.org/abs/2607.18147)
...
### [233] [Modeling rare-earth and energy materials supply chains under theoretical China-outer-Mongolia political reunification scenarios](https://arxiv.org/abs/2607.18019)
*Xi Liu,Wenxi Fang*

Main category: physics.comp-ph

TL;DR: 本研究构建了一个2026-2036年的动态局部均衡Stackelberg模型，分析中国与蒙古跨境矿产贸易对关键矿产资源供应安全的影响，评估不同合作情景下中国社会福利的变动。


<details>
  <summary>Details</summary>
Motivation: 应对中国在稀土、锂、铜和煤炭等关键资源上因与蒙古跨境贸易碎片化而导致的供应波动和资源安全风险。

Method: 建立包含蒙古矿产开采、包头稀土加工和全球其余需求三个节点的Stackelberg供应链模型，内生化矿产供给曲线、滞后资本积累、里昂惕夫生产函数、投资行为与最优出口税政策。

Result: 通过校准和模拟三种情景（现状贸易、深度资源整合、基础设施延迟）发现，深化中蒙资源合作可显著提升中国资源供应稳定性与长期社会福利。

Conclusion: 加强中蒙跨境矿产资源整合与基础设施建设对中国保障关键矿产供应安全和实现清洁能源转型具有战略意义。
...
### [234] [anyakrakusuma: A Python Library for Entropic Schrödinger Bridges on Idealized Geometries](https://arxiv.org/abs/2607.18184)
...
TL;DR: 本文介绍了一个名为anyakrakusuma的开源Python库，用于通过对数域Sinkhorn-Knopp迭代求解离散静态薛定谔桥问题，并重建两个经验点云之间的熵插值。


<details>
  <summary>Details</summary>
Motivation: 为了解决在高成本-正则化比率下传统方法数值不稳定的问题，提出一种稳定求解离散静态薛定谔桥问题的方法。

Method: 采用对数域Sinkhorn-Knopp迭代算法实现熵正则化的最优传输求解，并结合信息论与几何测度构建诊断流程以分析最优耦合与中间分布。

Result: 该方法在四个理想化平面案例中实现了高达10^{-9}的边际残差和单位边际保真度，残差历史在约八个数量级上呈几何衰减，且协方差分析能以极高精度恢复设定的旋转角度。

Conclusion: 对数域公式在极端参数条件下是必要的，所提方法具有高数值稳定性与精确性，适用于理想化构造案例，但应用于实际经验数据仍需进一步研究。
...
### [235] [Theoretical adversarial collaboration: a template](https://arxiv.org/abs/2607.16374)
*Kelvin J. McQueen,Markus P. Mueller*

Main category: physics.hist-ph

TL;DR: 本文提出了一种理论性对抗性合作的五步模板，旨在通过持续澄清、强化对方理论立场及共同构建异议，推动哲学与基础科学中难以通过实验解决的理论争议取得建设性进展。


<details>
  <summary>Details</summary>
Motivation: 针对无法通过实验轻易解决的理论争议，现有批判-回应-再回应的交流模式效果有限，因此需要一种更有效的合作框架来促进实质性学术进展。

Method: 提出了一个五步模板，包括：明确争议点、相互澄清、监督式强化对方理论（steel-manning）、协作提出异议、共同完善论点，并通过作者在哲学与物理学基础领域的实际对抗性合作实践验证该框架。

Result: 该模板能够超越传统的学术争论循环，产生显著超出已有文献的新见解，例如揭示理论如何应对新挑战，或促使理论倡导者承认需引入新的概念资源或修正理论。

Conclusion: 理论性对抗性合作是一种可行且富有成效的方法，可用于推动哲学、基础科学及其他非实验性领域中的理论争议向更具建设性和可发表的方向发展。
...
### [236] [Realism about the external world: an adversarial collaboration](https://arxiv.org/abs/2607.16379)
*Kelvin J. McQueen,Markus P. Mueller*

Main category: physics.hist-ph

TL;DR: 本文探讨了实在论与观念论之间的哲学分歧，特别是围绕“算法理想主义”这一形式化模型的连贯性及其解决观察者悖论（如玻尔兹曼大脑）的能力。两位作者通过对抗性合作的方式，澄清彼此观点，旨在促进跨学科对话并减少学术碎片化。


<details>
  <summary>Details</summary>
Motivation: 澄清关于现实本质的根本哲学分歧，尤其是物理实在论与理想论之间的对立，并检验算法理想主义是否能有效解决现代物理学中的观察者悖论。

Method: 采用对抗性协作方法，使用一种学科中立的理论协作模板，使持不同立场的研究者能在无误解的情况下系统地呈现分歧核心。

Result: 展示了实在论与理想论在解释观察者地位和物理定律基础时的根本差异；明确了算法理想主义作为理想论框架在解决悖论方面的潜力与争议点；实现了清晰、结构化的跨立场学术对话。

Conclusion: 真正的分歧在于形而上学预设与对形式模型的解释，而非技术细节；此类结构化对抗合作有助于推动哲学与科学领域的理性交流与整合。
...
### [237] [On the detection of absolute velocity in a Newtonian universe](https://arxiv.org/abs/2607.16584)
*Jorge Manero,Ricardo Muciño,Elias Okon*
...
### [238] [Particle accelerators as a path to intergovernmental collaboration: the case of the first electron positron collider and the power of theoretical physics](https://arxiv.org/abs/2607.16825)
*Giulia Pancheri*
...
### [240] [Effects of stochastic and natural seismic noise on the performance of waveform cross-correlation used to recover low-magnitude seismicity prior to the July 29, 2025, Kamchatka earthquake](https://arxiv.org/abs/2607.16226)
*Ivan Kitov*

Main category: physics.geo-ph

TL;DR: 利用波形互相关（WCC）技术处理IMS地震台阵数据，显著降低检测阈值，发现低震级地震活动模式可作为巨震前兆；通过随机噪声建模和侧向敏感性校正，验证了WCC事件的统计显著性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 旨在提升国际监测系统（IMS）对微弱地震信号的检测能力，探索低震级地震活动是否可作为巨震（如堪察加半岛潜在大地震）的前兆指标，并解决WCC方法在实际应用中面临的统计显著性不足与台阵侧向敏感性导致的假阳性问题。

Method: 采用波形互相关（WCC）技术处理IMS seismic array数据，使用随机噪声生成WCC目录以评估事件假设的统计显著性，并通过引入2011年东日本大地震的主事件（MEs）建模并校正IMS台阵的侧向敏感性问题；同时测试噪声幅度缩放对检测性能的影响，类比于噪声白化效应。

Result: WCC方法将检测阈值降低约一个数量级，识别出低于IDC报告震级至少一级的微小地震；随机噪声实验显示WCC事件极少，证实其统计显著性；加入Tohoku区域主事件后，由侧向敏感性引起的虚假事件消失；小幅缩放随机噪声可增强WCC检测能力，增加有效 detections 和事件产出，表现出类似噪声白化的优化效应。

Conclusion: WCC技术能有效揭示接近堪察加地震复发曲线拐点的低震级地震活动增强现象，该模式可能是巨震准备过程的前兆；结合随机噪声基准和多区域主事件冲突解析策略，可消除假象并确认WCC检测结果的可靠性，为地震前兆研究提供了高灵敏度工具。
...
### [241] [An Agentic Interface for End-to-End Probabilistic Seismic Hazard and Risk Analysis](https://arxiv.org/abs/2607.16249)
*Sreenath Vemula,Pierre Jehel,Fabrice Cotton,Filippo Gatti*

Main category: physics.geo-ph

TL;DR: 本文提出了一种基于Model Context Protocol（MCP）的开源代理式接口，用于端到端的概率性地震危险性与风险分析，通过大语言模型作为协调器，结合OpenQuake引擎实现快速、透明且可复现的计算，并支持用户自定义地面运动模型和多种高级功能。


<details>
  <summary>Details</summary>
Motivation: 现有的概率性地震危险性和风险分析系统主要由专家使用，缺乏开放、易用的工具；本文旨在构建一个可通过标准协议访问的开放平台，降低非专家用户的使用门槛，并提升分析过程的透明度与灵活性。

Method: 设计并实现了一个基于MCP协议的服务器接口，封装OpenQuake引擎和2020年欧洲地震危险性与风险模型，定义24个类型化端点；采用大语言模型作为代理（orchestrator），负责任务规划、翻译与解释，实际计算由OpenQuake引擎和自定义代码完成，确保结果具有数据溯源性。

Result: 在73个城市对EHS2020模型进行基准测试，475年重现期谱加速度估算中位误差仅为5%；完整从危险到损失评估可在数分钟内完成；支持用户上传经验或机器学习GMM，提供条件谱、确定性情景、地表危险、单体建筑损失、加固方案比较及波形记录选取等现有服务所缺少的功能。

Conclusion: 该框架成功实现了地震风险分析流程的自动化与大众化访问，兼具高精度与高效性，其分层设计可推广至其他区域模型与其他灾害类型，为未来智能灾害管理提供了可扩展的技术范式。
...
### [242] [Towards Open Science: Monitoring Crustal Deformations in North America](https://arxiv.org/abs/2607.16264)
...
### [243] [Observability of Finite-Depth Double-Diffusive Exchange from Sparse Temperature-Salinity Measurements](https://arxiv.org/abs/2607.16272)
*Sriram P. Kalathoor*
...
### [244] [Mantle-derived magmatic anhydrite in ultramafic rocks reveals deep mantle oxidation and mega-mineralization after Palaeozoic Oxygenation Event](https://arxiv.org/abs/2607.16883)
*Yuegao Liu*
...
### [245] [Multibath Influence Matrices: Universal Scaling from Real-Time Dynamics](https://arxiv.org/abs/2607.16411)
*Matan Lotem,Michael Sonner,Valentin Link,Alessio Lerose,Dmitry A. Abanin*

Main category: cond-mat.str-el

TL;DR: 提出了一种通过时空张量网络压缩模拟多库影响矩阵的方法，用于研究强关联多轨道系统的实时动力学，成功再现了双杂质安德森模型中的关键物理现象。


<details>
  <summary>Details</summary>
Motivation: 为解决临界动力学中不同时间尺度带来的经典与量子模拟瓶颈问题，需要发展高效的实时动力学计算方法。

Method: 将时空张量网络在时间上压缩为半群影响矩阵，在空间上使用矩阵积态进行压缩，并应用于双杂质安德森模型的多库影响矩阵模拟。

Result: 成功计算了谱函数，揭示了从近藤共振到非费米液体量子临界点再到绝缘单重态相的演化过程；在猝发淬火和Kibble-Zurek扫描中解析了瞬态到渐近动力学，并提取出普适的双通道近藤指数。

Conclusion: 多库影响矩阵结合张量网络压缩是一种有效且实用的工具，适用于强关联多轨道系统中真实时间动力学的研究。
...
### [246] [$J_{1} - J_{2} - δ$ model on a square lattice: From Altermagnet to Columnar antiferromagnet via quantum disordered phase](https://arxiv.org/abs/2607.16415)
*Subharthi Paul,Darshan G. Joshi*

Main category: cond-mat.str-el

TL;DR: 研究了在具有棋盘状次近邻相互作用的海森堡模型中，altermagnetism对抗由磁阻挫引起的量子涨落的鲁棒性，发现存在奈尔型altermagnet相、柱状反铁磁相以及二者之间的量子无序相。


<details>
  <summary>Details</summary>
Motivation: 探讨altermagnetism在没有自旋各向异性相互作用或外场的情况下，如何抵抗由磁阻挫引发的量子涨落。

Method: 采用线性自旋波理论，在正方晶格上构建包含近邻和两种次近邻相互作用的海森堡模型，研究不同相互作用强度下的磁相行为。

Result: 发现了Néel型altermagnet相和柱状反铁磁相，并识别出一个介于二者之间的量子无序相；该无序区域随着向棋盘格极限调节而更稳定。

Conclusion: Altermagnets在一定参数范围内对量子涨落具有鲁棒性，但强阻挫会导致量子无序相的出现，揭示了几何阻挫对altermagnetic有序的重要影响。
...
### [247] [Stroboscopic stability of a Floquet chiral spin liquid beyond the folding frequency](https://arxiv.org/abs/2607.16515)
*Didier Poilblanc*
...
### [248] [AIMS: An uncertainty-aware AI experimentalist for quantum matter](https://arxiv.org/abs/2607.16544)
...
### [249] [Tuning superconducting pairing symmetry via a staggered potential in the doped honeycomb Hubbard model](https://arxiv.org/abs/2607.16663)
...
### [250] [Nematic Wigner crystals in rhombohedral multilayer graphene](https://arxiv.org/abs/2607.17027)
...
### [251] [GPU-accelerated finite-temperature Lanczos method for Heisenberg spin systems with non-Abelian permutation symmetries](https://arxiv.org/abs/2607.17429)
...
### [252] [Gate-tunable giant anomalous Hall effect in magnetic topological insulator bilayer](https://arxiv.org/abs/2607.17988)
...
### [253] [Frustration induced dimensional reduction and coexistence of long and short-range magnetic order in NdCl$_{3}$](https://arxiv.org/abs/2607.17995)
...
### [254] [Phonon dynamics in Chromium under pressure: absence of phonon criticality at the approach of the quantum critical point](https://arxiv.org/abs/2607.18015)
...
### [255] [Universal Dynamic Scaling of 2D Quantum Ising Transition on the Fuzzy Sphere](https://arxiv.org/abs/2607.18028)
...
### [256] [Color superconductors and holon metals from doping a Fractional Chern insulator](https://arxiv.org/abs/2607.18238)
*Ya-Hui Zhang*
...
### [257] [Benchmarking Goodness-of-Fit and Calibration Algorithms for Logistic Regression Classifiers: A Large-Scale Simulation Study under Sparse Data](https://arxiv.org/abs/2607.16344)
*Ebrahim Khaled Ebrahim,Ahmed El-Kotory*

Main category: stat.ME

TL;DR: 本文对二元逻辑回归的拟合优度和校准检验进行了大规模仿真评估，提出了一个统一的分类体系，并通过开源R包ebram.gof实现了二十多种检验方法。研究发现，传统的Hosmer-Lemeshow检验表现不佳，而McCullagh、Osius-Rojek、le Cessie-van Houwelingen、Stute-Zhu和GiViTI校准检验在控制第一类错误和检验功效之间取得了最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 由于现代数据中连续预测变量和稀疏性的普遍存在，传统拟合优度检验（如Pearson卡方和偏差统计量）失效，且实践中广泛依赖软件内置的Hosmer-Lemeshow检验，缺乏系统性比较和实证支持。因此需要一个全面、可重复的评估框架来识别更可靠的替代方法。

Method: 构建了一个统一的拟合优度检验分类体系，在五种协变量分布和四种误设情景下进行大规模可重复仿真（每种设置10,000次重复），评估超过二十种检验方法的表现，指标包括第一类错误率和检验功效，并开发了开源R包ebram.gof支持复现。

Result: 多个经典检验表现出过高的第一类错误率或检验功效不足；McCullagh、Osius-Rojek、le Cessie-van Houwelingen、Stute-Zhu和GiViTI校准检验在正确控制显著性水平的同时具有更高功效，均优于普遍使用的Hosmer-Lemeshow检验；实际低出生体重数据分析显示，仅敏感检验结合校准曲线才能发现遗漏交互项的模型问题。

Conclusion: 应摒弃对Hosmer-Lemeshow检验的默认依赖，推荐使用经过验证的高功效检验方法，并结合校准曲线进行综合评估，本文结果为逻辑回归模型拟合诊断提供了基于证据的实践指南。

Abstract: Binary logistic regression is among the most widely used classification algorithms, yet a classifier is only trustworthy if its predicted probabilities are well calibrated. The classical checks -- the Pearson chi-square and deviance statistics -- break down precisely in the modern setting where predictors are continuous and the data are sparse (one covariate pattern per observation). Four decades of research have produced dozens of alternative goodness-of-fit and calibration algorithms, yet practitioners still default to the Hosmer-Lemeshow test because it ships with their software. This paper provides a unified taxonomy and a large-scale, reproducible simulation benchmark; more than twenty tests are implemented in the open-source R package ebrahim.gof. We evaluate them across five covariate distributions and four misspecification scenarios, with 10,000 replications each, measuring both Type I error and power. Several classical tests prove liberal, rejecting correct models far too often, while others have little power. A compact core -- McCullagh, Osius-Rojek, le Cessie-van Houwelingen, Stute-Zhu, and the GiViTI calibration test -- delivers the best balance of correct size and high power, and is consistently more powerful than the ubiquitous Hosmer-Lemeshow test. A low-birth-weight application reinforces the point: a model with omitted interactions slips past nearly every test, exposed only by pairing sensitive tests with a calibration (reliability) curve. We translate these findings into practical, evidence-based guidance for assessing logistic regression fit.

</details>


### [258] [Which Effect of Race? Causal Inference without Holding All Else Equal](https://arxiv.org/abs/2607.16371)
*Thomas Leavitt*

Main category: stat.ME

TL;DR: 本文探讨了种族歧视实证研究中的因果推断设计，提出随机化能够恢复一系列因果效应（包括固定非种族特征和允许相关特征随种族变化的效应），并重新分析了一项西班牙语竞选实验，发现在不同效应定义下对同族偏好的结论不同。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常在保持非种族特征不变的情况下操纵种族以进行因果推断，但作者认为这种设计所依赖的前提需要澄清：一是应估计何种“种族效应”，二是研究设计是否能准确识别该效应。本文旨在分离这两个问题，并重新审视因果识别的条件。

Method: 通过理论推导，展示相同的随机化设计可以识别一族因果效应，包括all-else-equal效应和within-race效应；并基于对西班牙语竞选实验的再分析，比较不同效应下的实证结果。

Result: 理论上证明了随机化可识别多个合理的因果效应，且这些效应均具有因果意义；实证上发现，在within-race效应下存在选民的同族偏好，而在all-else-equal效应下则无此现象。

Conclusion: 选择不同的种族效应对应于对种族类别本质的不同理解，研究者应明确其理论假设；放宽传统识别条件仍可实现可信的因果推断，这对种族及其他身份类别的研究具有广泛意义。
...
### [259] [Neural Networks of Outcome Weighted Learning for Individualized Treatment Rules](https://arxiv.org/abs/2607.16407)
*Zhu Wang*
...
### [260] [Claim-Specific Admissibility of PCA Biplot Interpretations: Target Alignment, Spectral Identifiability, and Projection Adequacy](https://arxiv.org/abs/2607.16469)
...
### [262] [Amortized Inference for Sampling Distributions Where the Bootstrap Fails](https://arxiv.org/abs/2607.16666)
*Akash Deep*
...
### [263] [Generalized reparametrized variational Bayes with skew-symmetric normalization](https://arxiv.org/abs/2607.16698)
...
### [265] [Decoupling risk and masking in mammographic density under irregular follow up using a latent Markov progression detection framework](https://arxiv.org/abs/2607.16793)
...
Abstract: Background: Mammographic density is a strong marker of breast cancer risk, yet it also reduces mammographic sensitivity through masking. Screening densities are observed at irregular times, requiring methods that accommodate irregular follow-up.
...
### [266] [Bayesian inference on the exponentiated Kumaraswamy distribution with applications](https://arxiv.org/abs/2607.16823)
*Ziqian Sun,Miguel A. Juárez*
...
Abstract: We discuss Bayesian estimation of the exponentiated Kumaraswamy distribution, based on a scale-location parameterisation. The parameterisation facilitates prior elicitation and interpretation of results, but potentially entails identifiability issues that are addressed trough a hierarchical weak informative prior setting. Our HMC implementation enables off-the-shelf utility for practitioners, and is illustrated on synthetic and real data.

</details>


### [267] [Non-Parametric Model Calibration with Stochastic Control Parameters](https://arxiv.org/abs/2607.16975)
...
### [268] [Adaptive Ridge-Regularized Hotelling Change-Point Tests for Functional Data](https://arxiv.org/abs/2607.16976)
*Ping Zhao,Long Feng*
...
### [272] [Calibrating Semantic Uncertainty from Observable Language-Model Probabilities](https://arxiv.org/abs/2607.17447)
*Matthew F. Dixon*
...
### [273] [Two-stage Adaptive Testing of Large-scale Mediation Hypotheses](https://arxiv.org/abs/2607.17579)
...
### [274] [E-Values For Multiplicity Control In Multiverse Analysis](https://arxiv.org/abs/2607.17596)
...
### [276] [Equality, Equity, and Causality in Fairness Research: A Commentary on Cheng (2026)](https://arxiv.org/abs/2607.17679)
*Youmi Suk*
...
### [277] [Fitting the topology of synthetic particle systems with a novel graph representation](https://arxiv.org/abs/2607.17680)
...
### [278] [A Globally Calibrated Bayesian Optimal Phase II Design for Adaptive Enrichment Trials](https://arxiv.org/abs/2607.17692)
...
### [280] [Comparing Missing Data Methods for Estimating Average Treatment Effects Under Time-Varying Confounding: A Simulation Study](https://arxiv.org/abs/2607.17775)
...
### [284] [Nanowire networks' interconnection graphs from their photomicrographs](https://arxiv.org/abs/2607.16445)
*Javier Tau Anzoátegui,Juan Ignacio Diaz Schneider,Eduardo D. Martínez,Pablo Levy,Oscar Filevich,Cynthia P. Quinteros*

Main category: cond-mat.dis-nn

TL;DR: 提出了一种从银纳米线网络的显微图像中提取连接图的优化流程，用于分析实验网络结构并评估其拓扑特性，发现其具有小世界和模块化特征，并研究了虚假交叉点对聚类系数和路径长度的影响。


<details>
  <summary>Details</summary>
Motivation: 为了分析银纳米线网络的实验架构并将其用于神经形态计算模拟，需要准确提取其互连结构。

Method: 通过对银纳米线网络的光学显微图像进行后处理，构建一个优化的管道来识别纳米线之间的交叉点，生成每个样本对应的图结构，并分析其拓扑指标。

Result: 实验组装的网络在度分布、邻接矩阵等分析下表现出与小世界和模块化拓扑相似的特性；同时发现顶视图图像会高估连接数，因此研究了人为移除连接对聚类系数和路径长度的影响。

Conclusion: 所提出的提取流程能有效获取银纳米线网络的互连图，有助于理解其拓扑属性，但需注意顶视图中虚假交叉点的影响，后续分析应考虑三维结构信息以提高准确性。
...
### [285] [Point Group Equivariant Graph Neural Networks for Materials](https://arxiv.org/abs/2607.16871)
*Alexander J. Heilman,Qimin Yan*

Main category: cond-mat.dis-nn

TL;DR: 本文提出了点群对称性感知的等变图神经网络（PGEqNN），通过将滤波函数与对称性感知索引对齐，提升了材料科学中张量弹性与介电性质预测的精度与效率。研究表明，大部分预测能力来源于点群基的平凡子空间，因此提出的A₁受限变体在更少参数下实现了与完整模型相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统O(3)等变图神经网络虽能处理任意原子结构，但未充分利用原子系统的内在点群对称性，且滤波函数的对称性依赖数据隐式学习而非显式约束，限制了模型效率与可解释性。

Method: 提出PGEqNN架构，引入点群对称性感知的滤波机制，使用点群适配的基函数对特征进行分解，并设计A₁-受限版本仅保留平凡表示子空间以减少参数量。

Result: 在弹性与介电张量数据集上验证表明，模型的主要预测能力集中在平凡子空间；A₁受限模型在训练参数更少的情况下，达到或超过全点群和SO(3)分解模型的精度。

Conclusion: 通过显式融合点群对称性，PGEqNN不仅提高了对材料张量属性预测的效率与性能，还揭示了等变模型中关键信息主要位于低维对称不变子空间，为构建轻量化、高解释性的材料机器学习模型提供了新路径。
...
### [286] [Transport-Weighted Coercivity in Granular CrO2: Why Magnetoresistance and Magnetization Can Disagree](https://arxiv.org/abs/2607.16984)
...
### [287] [Distinct reentrant transitions in a quasi-periodic Raman lattice](https://arxiv.org/abs/2607.17104)
...
### [288] [The Geometry of Semantic Space: A Continuous Geometric Framework for the Transformer Architecture](https://arxiv.org/abs/2607.17146)
*Zhihua Liang*
...
### [289] [Statistical-mechanics of a three-body Hopfield model with finite connectivity](https://arxiv.org/abs/2607.17234)
...
### [290] [Semi-fractality and localization on a chiral Cayley tree](https://arxiv.org/abs/2607.18179)
...
### [291] [A Measure-Theoretic Approach to Spontaneous Stochasticity](https://arxiv.org/abs/2607.16328)
*Wandrille Ruffenach,Eric Simonnet,Nicolas Valade*

Main category: nlin.CD

TL;DR: 本文提出了一种测度论框架来定义自发随机性（SpSt），将其理解为一种测度选择原理，并建立了有限维系统中非唯一性与任意概率测度可被选为极限律之间的联系，揭示了SpSt在湍流等系统中的生成机制。


<details>
  <summary>Details</summary>
Motivation: 自发随机性在湍流中是否普遍存在仍是一个开放问题，且缺乏普适的数学定义，本文旨在建立一个一般性的理论框架来阐明其本质。

Method: 引入测度论形式体系，研究正则化流对初始测度的前推，并分析其极限行为；结合奇点分析、初值敏感性及重正化群观点进行结构刻画。

Result: 证明了当无粘问题解不唯一时，任何支撑在无粘状态集上的概率测度均可通过适当正则化选出；识别出Dini型方向增长所揭示的奇点集是导致非唯一的必要障碍；提出了统计吸引子的概念以解释极限统计分布的形成。

Conclusion: 自发随机性可作为一类广泛存在的测度选择机制，在满足特定条件下可通过正则化过程实现统计选择，而非传统的确定性选择。
...
### [292] [Integrability-breaking phase transitions in stadium-like billiards](https://arxiv.org/abs/2607.16482)
*Anne Kétri P. da Fonseca,Edson D. Leonel*

Main category: nlin.CD

TL;DR: 研究了两类具有抛物边界的钱币形台球系统中破坏可积性的相变，发现聚焦边界导致连续相变，而散射边界导致一级相变，并建立了相应的标度律和临界现象框架。


<details>
  <summary>Details</summary>
Motivation: 探索哈密顿台球系统中从可积到非可积的转变机制，特别是相变类型及其普适性。

Method: 通过分析边界粗糙度ω的标度行为，结合相空间结构和动力学特征，区分两种不同类型的相变，并应用临界现象理论进行刻画。

Result: 发现了聚焦边界下的连续相变和散射边界下的一级相变，确定了各自的临界指数和标度律；对连续相变完整识别了对称性破缺、序参量、发散的 susceptibility、基本激发和拓扑缺陷。

Conclusion: 为哈密顿台球系统的可积性破缺相变建立了统计力学框架，表明临界现象的概念可自然推广至确定性非线性动力系统。
...
### [293] [Beyond the Edge of Chaos: Stability-Expressivity Transfer in Reservoir Forecasting](https://arxiv.org/abs/2607.17909)
*Yao Du,Xingang Wang*

Main category: nlin.CD

TL;DR: 该研究发现，用于储层计算的“混沌边缘”启发式方法在预测性能上的最优谱半径并不对应于孤立或闭环系统中的李雅普诺夫边缘。通过分析教师强制储层的集体动力学，提出了一种稳定性-可表达性传递指数，能准确识别自主预测的最优谱半径。


<details>
  <summary>Details</summary>
Motivation: 探讨‘混沌边缘’原则在储层计算机性能优化中的实际有效性，并寻找更准确的参数选择依据。

Method: 以储层网络的谱半径为控制参数，分析教师强制和闭环生成储层的李雅普诺夫边缘，并引入稳定性-可表达性转移指数来平衡模式稳定性和表达能力。

Result: 发现最佳预测性能对应的谱半径不位于传统意义上的混沌边缘；所提出的指数在不同类型目标（混沌、准周期）和储层结构下均能准确识别最优谱半径。

Conclusion: 传统的边沿混沌启发式并非最优指导原则，新提出的稳定性-可表达性转移指数是更有效的性能预测与参数优化工具。
...
### [294] [Chirality of a $Z_q$ Model as Directional Phase Shifts in Oscillator Networks](https://arxiv.org/abs/2607.16606)
*Yi Cheng,Zongli Lin*
...
### [295] [Geometry Induced Adaptive Dissipation in Non Linear Contact Hamiltonian Systems Theory and Application to Duffing Oscillator](https://arxiv.org/abs/2607.17055)
*Vinesh Vijayan,R Sathishkumar,D Kaleeswaran*
...
### [296] [Projective Maximum Entropy: Universality and Acceptance-Region Calibration](https://arxiv.org/abs/2607.16547)
*Hideitsu Hino*

Main category: math.ST

TL;DR: 本文在非负测度的射影空间上重新构建最大熵参考分布，提出了一个普适性定理，统一了多种熵的最大熵结果，并证明在给定马氏接受区域时可唯一确定形变参数，从而提供了一种从指定可接受区域构造有界支撑参考分布的原则性方法。


<details>
  <summary>Details</summary>
Motivation: 传统最大熵方法在归一化概率单纯形上构建，对非归一化统计模型不够自然，且无法直接解释如何由给定的可接受区域确定有界支撑参考分布的形变参数。因此需要一种更合适的框架来解决这些问题。

Method: 将最大熵问题构建在非负测度的射影空间上，利用线性矩约束下的单调变换不变性，分析Tsallis熵、Rényi熵、Hölder复合得分等多种函数的优化一致性，并通过马氏距离定义的接受区域确定形变参数。

Result: 1) 提出了一个普适性定理，表明相同归一化幂泛函的任意允许单调变换在线性矩约束下具有相同的最大熵优化器；2) 该优化器为q-指数密度，在均值和协方差约束下对应紧支撑的q-高斯或Student型密度；3) 给定马氏接受区域的平方半径R²>d+2时，可唯一确定形变参数γ_R=2/(R²−d−2)，对应的仿射等变参考密度是唯一的射影最大熵解，且其支撑与指定椭球一致。

Conclusion: 通过在射影空间上重构最大熵原理，本文为非归一化模型提供了更自然的框架，统一了解释多种熵的最大熵结果，并提出了一种由外部指定的可接受区域原则性地构造有界支撑参考分布的新方法。
...
### [297] [Identifiability of Partial-Mastery Cognitive Diagnostic Models](https://arxiv.org/abs/2607.16593)
*Jun Wu,Patrícia Martinková,Elena Erosheva*

Main category: math.ST

TL;DR: 本文提出了部分掌握认知诊断模型（PM-CDMs）的首个可识别性理论结果，通过符号计算和代数统计方法证明了在特定条件下模型参数具有通用有限对一可识别性。


<details>
  <summary>Details</summary>
Motivation: 尽管部分掌握认知诊断模型（PM-CDMs）受到越来越多关注，但其可识别性性质尚未被研究，这限制了模型在实际中的可靠估计与推断。

Method: 采用符号计算分析一个包含五个项目和两个潜属性的最小示例，验证模型参数化雅可比矩阵的非零性，并结合实分析与代数统计工具推导出局部性质到全局可识别性的扩展条件。

Result: 得到了PM-CDMs可识别性的充分条件，证明了项目参数及潜属性边际分布在局部结构满足条件时具有通用有限对一可识别性，且当Q矩阵包含所有属性对的此类可识别局部结构时，可识别性可推广至完整模型。

Conclusion: 本研究为部分掌握认知诊断模型的参数估计与统计推断提供了严格的理论基础，填补了该类模型在可识别性方面的理论空白。
...
### [298] [De-floored Principal Component Regression: When Rank Selection Alone Is Insufficient for Prediction](https://arxiv.org/abs/2607.16638)
*Peng Zhao*
...
### [299] [The Value of Depth in Message Passing on Sparse Graphs: A Kesten-Stigum Dichotomy](https://arxiv.org/abs/2607.16676)
*Aseem Raj Baranwal*
...
### [300] [Mirror and knockoff+ thresholds under dependence](https://arxiv.org/abs/2607.17084)
*Xianyang Zhang*
...
### [301] [Fundamental Limits of Query-Based Subgraph Detection](https://arxiv.org/abs/2607.17118)
*Wasim Huleihel*
...
### [302] [Matrix asymptotic calculus for plug-in maximum likelihood estimators in finite Markov chains](https://arxiv.org/abs/2607.17161)
...
### [303] [Efficient Doubly Adaptive Biased Coin Designs for Multiple Treatments](https://arxiv.org/abs/2607.17176)
*Zhang Li-Xin*
...
### [304] [Mixture and separation of log-concave measures](https://arxiv.org/abs/2607.17368)
...
### [305] [EM-based iterations for multiple instance learning on a query-value model](https://arxiv.org/abs/2607.17404)
*Ethan Levien*
...
### [306] [On Mixtures of Three Homoscedastic Gaussian Densities: An Unconditional Sharper Bound on the Number of Modes](https://arxiv.org/abs/2607.17506)
...
### [307] [A Functional Central Limit Theorem for Localized Partial Sums of Non-Stationary Time Series](https://arxiv.org/abs/2607.17697)
*Florian Heinrichs*
...
### [308] [Decision-Centric Large Deviations for Data-Driven Capital Buffers in Ruin Models](https://arxiv.org/abs/2607.17732)
...
### [309] [How Fast Do Signatures Learn? Statistical Theory and Applications for Path Regression](https://arxiv.org/abs/2607.17865)
...
### [310] [Unveiling Invariant and Transferable Latent Factors Across Heterogeneous Environments via ATLAS](https://arxiv.org/abs/2607.18209)
